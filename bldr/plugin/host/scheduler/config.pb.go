...
func (x *Config) GetExternalPluginIds() []string {
	if x != nil {
		return x.ExternalPluginIds
	}
	return nil
}
...
func (x *Config) MarshalProtoJSON(s *json.MarshalState) {
	...
	if len(x.ExternalPluginIds) > 0 || s.HasField("externalPluginIds") {
		s.WriteMoreIf(&wroteField)
...
func (x *Config) UnmarshalProtoJSON(s *json.UnmarshalState) {
	...
	s.ReadObject(func(key string) {
		...
		case "external_plugin_ids", "externalPluginIds":
			s.AddField("external_plugin_ids")
			if s.ReadNil() {
				x.ExternalPluginIds = nil
				return
			}
...
func (m *Config) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ExternalPluginIds) > 0 {
		for iNdEx := len(m.ExternalPluginIds) - 1; iNdEx >= 0; iNdEx-- {
			i = protobuf_go_lite.EncodeString(dAtA, i, m.ExternalPluginIds[iNdEx])
			i--
			dAtA[i] = 0x1
			i--
			dAtA[i] = 0x9a
...
func (m *Config) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 19:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ExternalPluginIds", wireType)
			}
			var v string
			v, iNdEx, err = protobuf_go_lite.DecodeString(dAtA, iNdEx)
			if err != nil {
