...
(() => {
	...
	function pd(event) {
		...
		if (!retv) throw new Error("'this' is expected an Event object, but got " + event);
		return retv;
	}
	function setCancelFlag(data) {
		...
		data.eventInit.cancelable && (data.canceled = !0);
	}
	var Event = class {
		constructor(eventType, eventInit = {}) {
			if (eventInit && typeof eventInit != "object") throw TypeError("Value must be an object.");
		...
		*/
		stopImmediatePropagation() {
			let data = pd(this);
			data.stopped = !0, data.immediateStopped = !0;
		}
		/**
	...
	function getListeners(eventTarget) {
		...
		if (!listeners) throw new TypeError("'this' is expected an EventTarget object, but got another value.");
		return listeners;
	}
	var EventTarget = class {
		...
		*/
		dispatchEvent(event) {
			if (typeof event != "object") throw new TypeError("Argument 1 of EventTarget.dispatchEvent is not an object.");
			if (!(event instanceof Event)) throw new TypeError("Argument 1 of EventTarget.dispatchEvent does not implement interface Event.");
...
var RemoteRPCError = class extends Error {
	rpcService;
	rpcMethod;
	rpcError;
...
function pipe(first, ...rest) {
	...
	if (fns.length > 2) {
		for (let i2 = 1; i2 < fns.length - 1; i2++) if (isDuplex(fns[i2])) fns[i2] = duplexPipelineFn(fns[i2]);
	}
	return rawPipe(...fns);
}
var rawPipe = (...fns) => {
...
var duplexPipelineFn = (duplex) => {
	return (source) => {
		...
		if (p?.then != null) {
			...
			if (isAsyncIterable2(source2)) sourceWrap = async function* () {
				yield* source2;
				stream.end();
			};
			else if (isIterable(source2)) sourceWrap = function* () {
				yield* source2;
				stream.end();
			};
...
function scalarEquals(type, a2, b) {
	...
	switch (type) {
		...
		case ScalarType.SINT64: return a2 == b;
	}
	return false;
}
...
function normalizeFieldInfos(fieldInfos, packedByDefault) {
	...
	for (const field of typeof fieldInfos == "function" ? fieldInfos() : fieldInfos) {
		...
		if (field.packed === void 0) if (packedByDefault) f2.packed = field.kind == "enum" || field.kind == "scalar" && field.T != ScalarType.BYTES && field.T != ScalarType.STRING;
		else f2.packed = false;
		if (field.oneof !== void 0) {
			const ooname = typeof field.oneof == "string" ? field.oneof : field.oneof.name;
			if (!o2 || o2.name != ooname) o2 = new InternalOneofInfo(ooname);
			f2.oneof = o2;
			o2.addField(f2);
		}
		r2.push(f2);
	...
	switch (value.kind) {
		case "scalar":
			for (const [k, v2] of Object.entries(sourceMap)) {
				throwSanitizeKey(k);
				if (v2 !== void 0) targetMap[k] = normalizeScalarValue(value.T, v2, clone);
		...
		case "enum":
			for (const [k, v2] of Object.entries(sourceMap)) {
				throwSanitizeKey(k);
				if (v2 !== void 0) targetMap[k] = normalizeEnumValue(value.T, v2);
				else delete targetMap[k];
			}
			break;
		case "message": {
	...
	skip(wireType) {
		...
		switch (wireType) {
			...
			case WireType.StartGroup:
				let t2;
				while ((t2 = this.tag()[1]) !== WireType.EndGroup) this.skip(t2);
				break;
...
function writeUnknownFields(message, writer) {
	const c2 = message[unknownFieldsSymbol];
...
function writeField2(field, value, options) {
	if (field.kind == "map") {
		...
		switch (field.V.kind) {
			case "scalar":
				for (const [entryKey, entryValue] of entries2) jsonObj[entryKey.toString()] = writeScalar2(field.V.T, entryValue);
				break;
			case "message":
			...
			case "enum": {
				...
				for (const [entryKey, entryValue] of entries2) jsonObj[entryKey.toString()] = writeEnum(enumType, entryValue, options.enumAsInteger);
				break;
			}
		}
	...
	if (field.repeated) {
		...
		if (valueArr && valueArr.length) switch (field.kind) {
			case "scalar":
				for (let i2 = 0; i2 < valueArr.length; i2++) jsonArr.push(writeScalar2(field.T, valueArr[i2]));
				break;
			case "enum":
				for (let i2 = 0; i2 < valueArr.length; i2++) jsonArr.push(writeEnum(field.T, valueArr[i2], options.enumAsInteger));
				break;
			case "message": {
				const messageType = resolveMessageType(field.T);
				for (let i2 = 0; i2 < valueArr.length; i2++) jsonArr.push(messageType.toJson(wrapField(messageType.fieldWrapper, valueArr[i2])));
				break;
			}
		}
...
function createMessageType(params, exts) {
	...
	const mt = {
		...
		fromJson(jsonValue, options) {
			const message = {};
			if (jsonValue != null) readMessage2(fields, typeName, jsonValue, makeReadOptions2(options), message);
			return message;
		},
		...
		toJson(a2, options) {
			return writeMessage2(a2, fields, makeWriteOptions2(options));
		},
...
var CommonRPC = class {
	...
	async handleCallData(packet) {
		...
		if (packet.error) this._rpcDataSource.end(new RemoteRPCError(this.service, this.method, packet.error));
...
var Uint8ArrayList = class _Uint8ArrayList {
	...
	*/
	write(buf, offset = 0) {
		...
		else if (isUint8ArrayList(buf)) for (let i2 = 0; i2 < buf.length; i2++) this.set(offset + i2, buf.get(i2));
...
async function* lengthPrefixEncode(source, lengthEncoder) {
	for await (const chunk of source) yield new Uint8ArrayList(lengthEncoder(chunk instanceof Uint8Array ? chunk.length : chunk.byteLength), chunk);
}
async function* lengthPrefixDecode(source, lengthDecoder) {
	...
	for await (const chunk of source) {
		...
		while (buffer.length >= lengthDecoder.bytes) {
			...
			yield buffer.sublist(lengthDecoder.bytes, totalLength);
...
function verifyConfig(config) {
	if (config.keepAliveInterval <= 0) throw new InvalidParametersError("keep-alive interval must be positive");
	if (config.maxInboundStreams < 0) throw new InvalidParametersError("max inbound streams must be larger or equal 0");
	if (config.maxOutboundStreams < 0) throw new InvalidParametersError("max outbound streams must be larger or equal 0");
	if (config.initialStreamWindowSize < INITIAL_STREAM_WINDOW) throw new InvalidParametersError("InitialStreamWindowSize must be larger or equal 256 kB");
	if (config.maxStreamWindowSize < config.initialStreamWindowSize) throw new InvalidParametersError("MaxStreamWindowSize must be larger than the InitialStreamWindowSize");
	if (config.maxStreamWindowSize > 2 ** 32 - 1) throw new InvalidParametersError("MaxStreamWindowSize must be less than equal MAX_UINT32");
	if (config.maxMessageSize < 1024) throw new InvalidParametersError("MaxMessageSize must be greater than a kilobyte");
}
var FrameType;
(function(FrameType2) {
...
function decodeHeader(data) {
	if (data[0] !== YAMUX_VERSION) throw new InvalidFrameError("Invalid frame version");
	return {
		type: data[1],
...
var Decoder = class {
	...
	*/
	...
	readHeader() {
		if (this.frameInProgress) throw new InvalidStateError("decoding frame already in progress");
...
var AbstractStream = class {
	...
	constructor(init) {
		...
		this.source = this.streamSource = pushable({ onEnd: (err) => {
	...
	async sink(source) {
		if (this.writeStatus !== "ready") throw new StreamStateError(`writable end state is "${this.writeStatus}" not "ready"`);
		try {
			this.writeStatus = "writing";
			const options = { signal: this.sinkController.signal };
			if (this.direction === "outbound") {
				const res = this.sendNewStream(options);
				if (isPromise2(res)) await res;
			}
			const abortListener = () => {
			...
			if (this.writeStatus === "writing") {
				...
				await this.sendCloseWrite({ signal: AbortSignal.timeout(this.sendCloseWriteTimeout) });
	...
	async closeWrite(options = {}) {
		...
		if (this.writeStatus === "writing") {
			if (this.sendingData != null) await raceSignal2(this.sendingData.promise, options.signal);
	...
	*/
	abort(err) {
		...
		if (isPromise2(res)) res.catch((err2) => {
...
function forEach(source, fn) {
	let index = 0;
	if (isAsyncIterable3(source)) return (async function* () {
		for await (const val of source) {
			const res2 = fn(val, index++);
			if (isPromise3(res2)) await res2;
			yield val;
		}
	})();
...
var YamuxStream = class extends AbstractStream {
	...
	*/
	async waitForSendWindowCapacity(options = {}) {
		...
		let reject;
		const abort = () => {
			if (this.status === "open" || this.status === "closing") reject(new AbortError3("Stream aborted"));
			else resolve();
		};
	...
	*/
	async handleData(header, readData) {
		...
		this.processFlags(header.flag);
		if (this.recvWindowCapacity < header.length) throw new ReceiveWindowExceededError("Receive window exceeded");
	...
	*/
	processFlags(flags) {
		...
		if ((flags & Flag.FIN) === Flag.FIN) this.remoteCloseWrite();
		if ((flags & Flag.RST) === Flag.RST) this.reset();
	}
	/**
	...
	*/
	sendWindowUpdate() {
		if (this.readStatus !== "ready") return;
		const flags = this.getSendFlags();
		const now = Date.now();
		const rtt = this.getRTT();
		if (flags === 0 && rtt > -1 && now - this.epochStart < rtt * 4) this.recvWindow = Math.min(this.recvWindow * 2, this.config.maxStreamWindowSize);
...
var YamuxMuxer = class {
	...
	constructor(components, init) {
		...
		this.sink = async (source) => {
			const shutDownListener = () => {
				...
				if (iterator.return != null) {
					...
					if (isPromise4(res)) res.catch((err) => {
		...
		if (this.config.enableKeepAlive) this.keepAliveLoop().catch((e2) => this.log?.error("keepalive error: %s", e2));
	...
	newStream(name) {
		...
		this.nextStreamID += 2;
		if (this.numOutboundStreams >= this.config.maxOutboundStreams) throw new TooManyOutboundProtocolStreamsError("max outbound streams exceeded");
	...
	*/
	async ping() {
		...
		if (this.activePing === void 0) {
			...
			const end = Date.now();
			this.rtt = end - start;
		} else await this.activePing.promise;
		return this.rtt;
	}
	/**
	...
	*/
	...
	_newStream(id, name, state, direction) {
		if (this._streams.get(id) != null) throw new InvalidParametersError("Stream already exists with that id");
	...
	*/
	...
	async handleStreamMessage(header, readData) {
		...
		if (stream === void 0) {
			if (type === FrameType.Data) {
				...
				if (readData === void 0) throw new Error("unreachable");
				await readData();
			} else this.log?.trace("frame for missing stream id=%s", streamID);
			return;
		}
		switch (type) {
			case FrameType.WindowUpdate:
	...
	incomingStream(id) {
		if (this.client !== (id % 2 === 0)) throw new InvalidParametersError("Both endpoints are clients");
...
function combineUint8ArrayListTransform() {
	return async function* decodeMessageSource(source) {
		for await (const obj of source) if (isUint8ArrayList(obj)) yield obj.subarray();
		else yield obj;
	};
}
...
var StreamConn = class {
	...
	async openStream() {
		return streamToPacketStream(await this.muxer.newStream());
	}
	buildOpenStreamFunc() {
	...
	handleIncomingStream(strm) {
		...
		if (!server) return strm.abort(/* @__PURE__ */ new Error("server not implemented"));
		server.handlePacketStream(streamToPacketStream(strm));
	}
	close(err) {
		if (err) this.muxer.abort(err);
		else this.muxer.close();
	}
};
...
var HandleStreamCtr = class extends ValueCtr {
	...
	get handleStreamFunc() {
		return async (stream) => {
			...
			if (!handleFn) handleFn = await this.wait();
...
function writeCompleteChunk(os, fd, data) {
	...
	while (offset < data.length) {
		const bytesWritten = os.write(fd, data.buffer, data.byteOffset + offset, data.length - offset);
		if (bytesWritten < 0) throw new Error(`Write failed with error code: ${bytesWritten}`);
		if (bytesWritten === 0) throw new Error("Write returned 0 bytes, possible full disk or broken pipe");
		offset += bytesWritten;
	}
}
...
function createAbortController() {
	class AbortSignalImpl {
		...
		static timeout(delay) {
			const signal = new AbortSignalImpl();
			setTimeout(() => {
				signal._abort(/* @__PURE__ */ new Error("TimeoutError"));
			}, delay);
			return signal;
		}
		_aborted = false;
		...
		dispatchEvent(event) {
			if (event.type === "abort") {
				if (this._onabort) this._onabort(event);
		...
		_abort(reason) {
			...
			this._aborted = true;
			this._reason = reason !== void 0 ? reason : /* @__PURE__ */ new Error("AbortError");
...
function createSymbolPolyfills() {
	...
	if (!Symbol.dispose) Object.defineProperty(Symbol, "dispose", {
		value: /* @__PURE__ */ Symbol("Symbol.dispose"),
		writable: false,
		enumerable: false,
		configurable: false
	});
	if (!Symbol.asyncDispose) Object.defineProperty(Symbol, "asyncDispose", {
		value: /* @__PURE__ */ Symbol("Symbol.asyncDispose"),
		writable: false,
		enumerable: false,
		configurable: false
...
function stringToCodePoints(string) {
	...
	while (i2 < n2) {
		...
		if (c2 < 55296 || c2 > 57343) u2.push(c2);
		else if (56320 <= c2 && c2 <= 57343) u2.push(65533);
		else if (55296 <= c2 && c2 <= 56319) if (i2 === n2 - 1) u2.push(65533);
		else {
...
function codePointsToString(code_points) {
	...
	for (var i2 = 0; i2 < code_points.length; ++i2) {
		...
		if (cp <= 65535) s2 += String.fromCharCode(cp);
		else {
			cp -= 65536;
...
*/
handler: function(_stream, _code_point) {} };
var DEFAULT_ENCODING = "utf-8";
function TextDecoder2(encoding, options) {
	if (!(this instanceof TextDecoder2)) return new TextDecoder2(encoding, options);
	encoding = encoding !== void 0 ? String(encoding).toLowerCase() : DEFAULT_ENCODING;
	if (encoding === "utf8") encoding = DEFAULT_ENCODING;
...
function format(f2) {
	if (!isString(f2)) {
		var objects = [];
		for (let i3 = 0; i3 < arguments.length; i3++) objects.push(inspect(arguments[i3]));
		return objects.join(" ");
	}
	let i2 = 1;
	var args = arguments;
	...
	for (var x = args[i2]; i2 < len; x = args[++i2]) if (x === null || !inspectPassthroughTypes.has(typeof x)) str += " " + x;
	else str += " " + inspect(x);
	return str;
}
...
function formatValue(ctx, value, recurseTimes) {
	...
	let keys = descriptorsArr.flatMap(([v2, desc]) => desc.enumerable ? [v2] : []);
	const visibleKeys = new Set(keys);
	if (ctx.showHidden) keys = descriptorsArr.map(([v2, _desc]) => v2);
	if (keys.length === 0) {
	...
	if (typeof value === "function") base = " [Function" + (value.name ? ": " + value.name : "") + "]";
...
function formatPrimitive(ctx, value) {
	if (value === void 0) return ctx.stylize("undefined", "undefined");
	if (isString(value)) {
	...
	if (isSymbol(value)) return ctx.stylize(value.toString(), "symbol");
}
...
function formatKey(ctx, key, visible) {
	let str = visible ? "" : "[";
	if (typeof key === "symbol") str += ctx.stylize("[" + formatValue(ctx, key, null) + "]", "special");
	else if (key.match(/^([a-zA-Z_][a-zA-Z_0-9]*)$/)) str += ctx.stylize(key, "name");
	else str += ctx.stylize("'" + JSON.stringify(key).slice(1, -1).replace(/\\"/g, "\"") + "'", "string");
	if (!visible) str += "]";
	return str;
}
function formatProperty(ctx, value, recurseTimes, visibleKeys, key, array) {
	...
	if (!str) if (ctx.seen.indexOf(desc.value) < 0) {
		if (recurseTimes === null) str = formatValue(ctx, desc.value, null);
	...
	if (name === void 0) {
		if (array && typeof key === "string" && key.match(/^\d+$/)) return str;
...
function createConsole({ logger, clearConsole, printer, formatter = (args) => format(...args), inspect: inspect2 = inspect }) {
	...
	if (!logger) logger = function Logger(logLevel, args, options) {
		if (args.length === 1) _printer(logLevel, args, options);
		else if (args.length > 1) _printer(logLevel, [formatter(args)], options);
	};
	let groupCount = 0;
	...
	consoleObj.table = function(data, properties) {
		if (properties !== void 0 && !Array.isArray(properties)) throw new Error("The 'properties' argument must be of type Array. Received type string");
		if (data === null || typeof data !== "object") return _printer("table", data);
		function getProperties(data2) {
			const props = [];
			const propsS = /* @__PURE__ */ new Set();
			for (const i2 in data2) if (typeof data2[i2] === "object") {
				for (const key in data2[i2]) if (!propsS.has(key)) {
					props.push(key);
					propsS.add(key);
				}
			}
		...
		if (!properties) properties = getProperties(data);
		function normalize(data2) {
			return inspect2(data2).replace(/\u001b\[\d\d?m/g, "");
		}
		...
		function getTableData(data2, properties2, addIndex = true) {
			...
			for (const i2 in data2) {
				...
				for (const p of properties2) row.push(normalize(data2[i2][p] || ""));
				rows2.push(row);
			}
			return rows2;
		}
		...
		const cols = [];
		for (let ci = 0; ci < rows[0].length; ci++) for (let ri = 0; ri < rows.length; ri++) cols[ci] = { width: Math.max(cols[ci]?.width ?? 0, countBytes(rows[ri][ci])) };
		function renderTable(rows2, cols2) {
			...
			function drawRow(row) {
				for (let ci = 0; ci < cols2.length; ci++) {
					if (ci === 0) str += tableChars.left;
					else str += tableChars.middle;
					str += " " + row[ci] + " ".repeat(cols2[ci].width - countBytes(row[ci]) + 1);
				}
				str += tableChars.right + "\n";
			}
			for (let ri = 0; ri < rows2.length; ri++) {
				if (ri === 0) drawHorizLine(tableChars.topLeft, tableChars.topRight + "\n", tableChars.topMiddle);
				else if (ri === 1) drawHorizLine(tableChars.leftMiddle, tableChars.rightMiddle + "\n", tableChars.rowMiddle);
				drawRow(rows2[ri]);
			}
	...
	consoleObj.trace = function(...data) {
		const stack = (/* @__PURE__ */ new Error()).stack.trim().split("\n").slice(1).join("\n");
	...
	consoleObj.countReset = function(label = "default") {
		if (!countMap.delete(label)) logger("countReset", ["countReset: No counter named " + label], { isWarn: true });
	};
	consoleObj.group = function(...data) {
		if (data.length > 0) logger("group", data);
		groupCount++;
	};
	...
	consoleObj.time = function(label = "default") {
		label = String(label);
		if (timers.has(label)) logger("time", ["Timer " + label + " already exists"], { isWarn: true });
		else timers.set(label, performance.now());
	};
	consoleObj.timeLog = function(label = "default", ...data) {
		label = String(label);
		if (!timers.has(label)) logger("timeLog", ["timeLog: No such timer: " + label], { isWarn: true });
		else {
	...
	consoleObj.timeEnd = function(label = "default") {
		label = String(label);
		if (!timers.has(label)) logger("timeEnd", ["timeEnd: No such timer: " + label], { isWarn: true });
		else {
...
function createQuickjsConsole(originalConsole) {
	return createConsole({
		...
		printer(logLevel, args, { indent = 0, isWarn = false }) {
			...
			const output = " ".repeat(indent * 2) + msg;
			if ([
				"error",
				"trace",
				"warn"
			].includes(logLevel) || isWarn) originalConsole.log("ERROR:", output);
...
function measure(name, startMark, endMark) {
	let startTime;
	let endTime;
	if (endMark !== void 0 && marksIndex[endMark] === void 0) throw new SyntaxError("Failed to execute 'measure' on 'Performance': The mark '" + endMark + "' does not exist.");
	if (startMark !== void 0 && marksIndex[startMark] === void 0) throw new SyntaxError("Failed to execute 'measure' on 'Performance': The mark '" + startMark + "' does not exist.");
	if (marksIndex[startMark]) startTime = marksIndex[startMark].startTime;
	else startTime = 0;
	if (marksIndex[endMark]) endTime = marksIndex[endMark].startTime;
	else endTime = globalThis.performance.now();
	const mark2 = {
		name,
		entryType: "measure",
		startTime,
...
function clearMarks(name) {
	if (typeof name === "undefined") entries = entries.filter((entry) => entry.entryType !== "mark");
	else {
		const entry = entries.find((e2) => e2.entryType === "mark" && e2.name === name);
		entries.splice(entries.indexOf(entry), 1);
		delete marksIndex[name];
	}
}
function clearMeasures(name) {
	if (typeof name === "undefined") entries = entries.filter((entry) => entry.entryType !== "measure");
	else {
		const entry = entries.find((e2) => e2.entryType === "measure" && e2.name === name);
		entries.splice(entries.indexOf(entry), 1);
	}
}
...
function btoa(s2) {
	...
	let i2;
	s2 = `${s2}`;
	for (i2 = 0; i2 < s2.length; i2++) if (s2.charCodeAt(i2) > 255) throw new DOMException("The string to be encoded contains characters outside of the Latin1 range.", "InvalidCharacterError");
	let out = "";
...
function installRetainedSchedulerPolyfills(target) {
	const roots = {
		microtasks: /* @__PURE__ */ new Set(),
		timeouts: /* @__PURE__ */ new Map(),
		intervals: /* @__PURE__ */ new Map()
	};
	const queueMicrotask2 = target.queueMicrotask?.bind(target) ?? ((func) => Promise.resolve().then(func));
	target.queueMicrotask = (func) => {
		const retained = () => {
			try {
				func();
			} finally {
		...
		queueMicrotask2(retained);
	};
	const setTimeout2 = target.os.setTimeout.bind(target.os);
	const clearTimeout2 = target.os.clearTimeout.bind(target.os);
	target.setTimeout = (func, delay) => {
		const retained = () => {
			try {
				func();
			} finally {
		...
		const handle = setTimeout2(retained, delay);
		roots.timeouts.set(handle, retained);
		return handle;
	};
	target.clearTimeout = (handle) => {
		roots.timeouts.delete(handle);
		clearTimeout2(handle);
	};
	const setInterval = target.os.setInterval.bind(target.os);
	const clearInterval2 = target.os.clearInterval.bind(target.os);
	target.setInterval = (func, delay) => {
		const retained = () => {
			func();
		};
...
var ControllerStatus_Enum = createEnumType("controller.exec.ControllerStatus", [
	...
	{
		no: 1,
		name: "ControllerStatus_CONFIGURING"
	},
	{
		no: 2,
		name: "ControllerStatus_RUNNING"
	},
	{
		no: 3,
...
var Retry = class {
	constructor(fn, opts) {
		...
		if (this._abortSignal) {
			...
			this._abortSignal.addEventListener("abort", this._abortListener, { once: true });
		}
	...
	cancel() {
		...
		if (this._reject) this._reject(this._currError);
	}
	async _execute() {
		do
			try {
	...
	_clearAbortListener() {
		if (!this._abortSignal || !this._abortListener) return;
		this._abortSignal.removeEventListener("abort", this._abortListener);
		this._abortListener = void 0;
	}
};
...
(function(t2) {
	function e2(n2, r2) {
		...
		return void 0 === r2 && (r2 = {}), (e3 = t2.call(this) || this).nn = {}, e3.tn = 0, e3.rn = new u(), e3.en = new u(), e3.on = new u(), e3.un = 0, e3.an = /* @__PURE__ */ new Set(), e3.cn = function() {
			...
			var t3 = e3.fn, r3 = n3.target, i3 = r3.state, o3 = r3 === e3.vn, u2 = {
				sw: r3,
				isExternal: o3,
				originalEvent: n3
			};
			...
			e3.dispatchEvent(new c("controlling", {
				isExternal: r3,
				originalEvent: n3,
				sw: t3,
				isUpdate: e3.mn
			...
			return f(e3.getSW(), (function() {
				e3.an.has(i3) && e3.dispatchEvent(new c("message", {
					data: t3,
					originalEvent: n3,
					ports: r3,
					sw: i3
	...
	var o2, s2 = t2;
	(o2 = e2).prototype = Object.create(s2.prototype), o2.prototype.constructor = o2, i(o2, s2);
	var l2 = e2.prototype;
			...
			return f((function(n3, t4) {
				...
				return e3.mn = Boolean(navigator.serviceWorker.controller), e3.dn = e3.pn(), f(e3.bn(), (function(n3) {
					...
					return t4 && a(t4.scriptURL, e3.sn.toString()) && (e3.hn = t4, Promise.resolve().then((function() {
						e3.dispatchEvent(new c("waiting", {
							sw: t4,
							wasWaitingBeforeRegister: true
						}));
					})).then((function() {}))), e3.hn && (e3.rn.resolve(e3.hn), e3.an.add(e3.hn)), e3.fn.addEventListener("updatefound", e3.cn), navigator.serviceWorker.addEventListener("controllerchange", e3.yn), e3.fn;
				}));
			})));
		} catch (n3) {
	...
	return t2.addEventListener = function(n3, t3) {
		...
		n3.target = this;
		for (var t3, r2 = e(this.jn(n3.type)); !(t3 = r2()).done;) (0, t3.value)(n3);
	}, t2.jn = function(n3) {
...
var releasedResourceClient = new Proxy({ released: true }, { get(target, prop) {
	if (prop === "released") return true;
	if (prop === "toJSON") return () => ({ released: true });
	const propStr = String(prop);
	if (typeof prop === "symbol" || propStr.startsWith("$$") || propStr.startsWith("@@__") || prop === "constructor" || prop === "prototype" || prop === "__proto__" || prop === "then" || prop === "asymmetricMatch" || prop === "nodeType" || prop === "tagName") return;
	throw new ResourceClientError(`Cannot access "${propStr}" on released resource`, "INVALID_RESOURCE");
} });
...
function createResourceRef(id, client, onRelease) {
	...
	const ref = {
		...
		get client() {
			if (released) return releasedResourceClient;
			return getSrpcClient();
		},
		createRef(newId) {
			if (released) throw new ResourceClientError(`Cannot create ref from released resource ${id}`, "INVALID_RESOURCE");
			return client.createResourceReference(newId);
		},
		createResource(newId, ResourceClass, ...args) {
			return new ResourceClass(this.createRef(newId), ...args);
		},
		release,
...
var Client2 = class {
	...
	*/
	...
	async attachRawInvoker(label, mux, signal) {
		return this.attachResource(label, mux, signal);
	}
	async attachResourceTree(label, mux, signal, releaseFn) {
		return this.attachResource(label, mux, signal, releaseFn);
	}
	async attachResource(label, mux, signal, releaseFn) {
		let attempt = 0;
		for (;;) {
			...
			try {
				...
				if (releaseFn) sess.releaseFns.set(resourceId, releaseFn);
				const cleanup = () => {
				...
				if (signal?.aborted || attempt >= 3 || !this.shouldRetryAttachResource(err, sess)) throw err;
				attempt++;
			}
		}
	}
	...
	async openAttachSession() {
		...
		if (ackBody.value.error) {
			...
			if (ackBody.value.error === resourceAttachClientNotFound) {
				...
				throw new ResourceClientError("Resource attach client was released", "CONNECTION_FAILED", err);
			}
			throw err;
		}
		const sess = {
		...
		const conn = new StreamConn(new Server(routedLookup), {
			direction: "inbound",
		...
		pipe((async function* () {
			for (;;) {
				...
				if (body?.case === "muxData") yield body.value;
					...
					if (!pending) continue;
					if (pending.canceled) {
						if (!addAck.error) outgoing.push({ body: {
							case: "detach",
							value: { resourceId: addAck.resourceId ?? 0 }
						} });
						continue;
					}
					if (!addAck.error) pending.resolve(addAck.resourceId ?? 0);
					else pending.reject(new Error(addAck.error));
				}
				if (body?.case === "detachAck") releaseAttachedResource(body.value.resourceId ?? 0);
			}
		})(), conn, combineUint8ArrayListTransform(), async (source) => {
			for await (const chunk of source) outgoing.push({ body: {
				case: "muxData",
				value: chunk
			} });
			outgoing.end();
	...
	*/
	async ensureInitialized() {
		this.throwIfDisposed();
		if (this.initState) return this.initState;
	...
	async performInitialization() {
		...
		return new Promise((resolve, reject) => {
			...
			const handleCancel = () => {
				if (!initialized) {
					cleanup();
					reject(new ResourceClientError("Client initialization was cancelled", "CLIENT_CANCELLED"));
				}
			};
			...
			this.startConnection(resolve, reject, () => {
				...
				cleanup();
				if (!initialized) reject(new ResourceClientError("Failed to initialize client connection", "CONNECTION_FAILED", error instanceof Error ? error : new Error(String(error))));
			});
		});
	}
	/**
	...
	*/
	async startConnection(onInitialized, onError, markInitialized) {
		...
		if (!controller) throw new ResourceClientError("No connection controller", "CONNECTION_FAILED");
		await retryWithAbort(controller.signal, async (signal) => {
			const attemptController = createAbortController2(signal);
			let initialized = false;
			...
			const throwInitTimeout = () => {
				throw new ResourceClientError(resourceClientInitTimeoutMessage, "CONNECTION_FAILED");
			};
			...
			try {
				const stream = this.service.ResourceClient({}, attemptController.signal);
				for await (const msg of stream) {
					...
					if (msg.body?.case === "clientError") {
						const error = new ResourceClientError(`Server error: ${msg.body.value}`, "SERVER_ERROR");
						if (!this.initState) onError(error);
						throw error;
					}
				}
			} catch (err) {
				...
				clearInitTimeout();
				if (this.connectionController === controller && this.resources.size > 0) this.releaseAllResources("connection-lost");
			}
			if (!signal.aborted) {
				if (initTimedOut) throwInitTimeout();
				throw new Error(initialized ? "ResourceClient stream closed" : "ResourceClient stream closed before init");
			}
		}, { errorCb: (err) => {
	...
	*/
	throwIfDisposed() {
		if (this.disposed) throw new ResourceClientError("Client has been disposed", "CLIENT_DISPOSED");
	}
	/**
	...
	*/
	createRef(id) {
		this.throwIfDisposed();
		if (!this.resources.has(id)) this.resources.set(id, /* @__PURE__ */ new Set());
	...
	*/
	enqueueServerResourceRelease(resourceId) {
		if (!this.initState || this.disposed) return;
		const pending = {
			clientHandleId: this.initState.clientHandleId,
			resourceId
		};
		const key = this.buildPendingResourceReleaseKey(pending.clientHandleId, pending.resourceId);
	...
	*/
	schedulePendingResourceReleaseFlush() {
		if (this.pendingResourceReleaseTask || this.disposed || this.pendingResourceReleases.size === 0) return;
		const controller = createAbortController2(this.signal);
		this.pendingResourceReleaseController = controller;
		this.pendingResourceReleaseTask = this.flushPendingResourceReleases(controller.signal).catch(() => {}).finally(() => {
			if (this.pendingResourceReleaseController === controller) this.pendingResourceReleaseController = null;
			this.pendingResourceReleaseTask = null;
			if (!this.disposed && this.pendingResourceReleases.size > 0) this.schedulePendingResourceReleaseFlush();
		});
	}
	/**
	...
	*/
	async flushPendingResourceReleases(signal) {
		while (!signal.aborted && !this.disposed && this.pendingResourceReleases.size) {
			...
			await retryWithAbort(signal, async (retrySignal) => {
				if (await this.tryNotifyServerResourceRelease(pending, retrySignal) === "retry") throw new Error("retry pending resource release");
			}, {
	...
	async tryNotifyServerResourceRelease(pending, signal) {
		...
		try {
			...
			if (error instanceof Error) {
				...
				if (msg.includes("ERR_RPC_ABORT")) return "done";
				if (msg.includes("resource not found") || msg.includes("invalid client id")) return "done";
				if (this.shouldRetryServerResourceRelease(error)) return "retry";
				console.warn(`Failed to notify server of resource ${pending.resourceId} release:`, error);
				return "done";
			}
			console.warn(`Failed to notify server of resource ${pending.resourceId} release:`, String(error));
			return "done";
		}
	}
	/**
	...
	*/
	...
	shouldRetryResourceClientStreamSilently(error) {
		if (error instanceof ResourceClientError && error.message === resourceClientInitTimeoutMessage) return true;
		...
		return (typeof errName === "string" ? errName : "") === "StreamResetError" || (typeof errMessage === "string" ? errMessage : "") === "stream reset";
	}
	/**
	...
	*/
	...
	restartConnectionAfterStaleAttachClient() {
		const controller = this.connectionController;
		this.connectionController = null;
		this.clearPendingResourceReleases();
		this.clearAttachSession();
...
var ResourceServiceClient = class {
	...
	*/
	ResourceClient(request, abortSignal) {
		...
		const result = this.rpc.serverStreamingRequest(this.service, ResourceServiceDefinition.methods.ResourceClient.name, ResourceClientRequest.toBinary(requestMsg), abortSignal || void 0);
	...
	*/
	async ResourceRefRelease(request, abortSignal) {
		...
		const result = await this.rpc.request(this.service, ResourceServiceDefinition.methods.ResourceRefRelease.name, ResourceRefReleaseRequest.toBinary(requestMsg), abortSignal || void 0);
	...
	*/
	ResourceAttach(request, abortSignal) {
		const result = this.rpc.bidirectionalStreamingRequest(this.service, ResourceServiceDefinition.methods.ResourceAttach.name, buildEncodeMessageTransform(ResourceAttachRequest)(request), abortSignal || void 0);
...
var PluginHostClient = class {
	...
	*/
	async GetPluginInfo(request, abortSignal) {
		...
		const result = await this.rpc.request(this.service, PluginHostDefinition.methods.GetPluginInfo.name, GetPluginInfoRequest.toBinary(requestMsg), abortSignal || void 0);
	...
	*/
	ExecController(request, abortSignal) {
		...
		const result = this.rpc.serverStreamingRequest(this.service, PluginHostDefinition.methods.ExecController.name, ExecControllerRequest.toBinary(requestMsg), abortSignal || void 0);
	...
	*/
	LoadPlugin(request, abortSignal) {
		...
		const result = this.rpc.serverStreamingRequest(this.service, PluginHostDefinition.methods.LoadPlugin.name, LoadPluginRequest.toBinary(requestMsg), abortSignal || void 0);
...
var BackendApiImpl = class {
	...
	utils = {
		pluginHttpPath: (pluginId, ...httpPaths) => {
			...
			if (httpPaths.length === 0 || !httpPaths[0].startsWith("/")) result += "/";
			for (const httpPath of httpPaths) result += httpPath;
			return result;
		},
		pluginDistHttpPath: (pluginId, httpPath) => {
			let result = this.constants.PLUGIN_DIST_HTTP_PREFIX + pluginId;
			if (!httpPath.startsWith("/")) result += "/";
			result += httpPath;
			return result;
		},
	...
	buildPluginOpenStream(pluginID, instanceKey) {
		...
		if (instanceKey) componentID += "/" + instanceKey;
...
function logError(message, err) {
	...
	if (err && typeof err === "object" && "stack" in err) console.error(err.stack);
}
var quickjsGlobalThis = globalThis;
function retainRuntimeRoot(value) {
...
var runtimeConn = new StreamConn({ handlePacketStream: handleIncomingStream }, {
	direction: "inbound",
	yamuxParams: {
		enableKeepAlive: false,
...
async function startPlugin() {
	...
	if (typeof script.default !== "function") throw new Error(`shared-worker: Imported module "${scriptPath}" does not have a default export function.`);
	let startInfo;
	if (startInfoB64) {
	...
	const backendAPI = new BackendApiImpl(startInfo, openStream, handleIncomingStreamCtr, abortSignal);
	const pluginLifetime = new Promise(() => {});
	retainRuntimeRoot({
		script,
		backendAPI,
		abortController,
