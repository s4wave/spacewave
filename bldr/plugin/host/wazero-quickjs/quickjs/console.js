...
// See: https://github.com/saghul/txiki.js/blob/6e01bf4/src/js/polyfills/console.js
...
function createConsole({
  ...
  consoleObj.table = function (data, properties) {
    ...
    function getTableData(data, properties, addIndex = true) {
      const rows = [addIndex ? ['(index)', ...properties] : [...properties]];
    ...
    function renderTable(rows, cols) {
      ...
      function drawRow(row) {
        for (let ci = 0; ci < cols.length; ci++) {
          ...
          str +=
            ' ' +
            row[ci] +
            ' '.repeat(cols[ci].width - countBytes(row[ci]) + 1);
        }
      ...
      for (let ri = 0; ri < rows.length; ri++) {
        if (ri === 0) {
          ...
          // only draw the middle line after the header
          drawHorizLine(
...
function createQuickjsConsole(originalConsole) {
  return createConsole({
    clearConsole() {
      // Simple clear implementation for QuickJS
    ...
    printer(logLevel, args, { indent = 0, isWarn = false }) {
      const msg = args
        .map((arg) => {
          if (typeof arg === 'string') {
            ...
            // Simple object inspection for QuickJS
            try {
      ...
      // Use the original console.log for output
...
export {
  createConsole,
  createQuickjsConsole,
  utilInspect as inspect,
  utilFormat as format,
};
