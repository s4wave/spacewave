...
const ControllerID = "bldr/web/plugin/handle-web-pkg-assets"
...
func (c *Controller) Execute(ctx context.Context) (rerr error) {
	...
	return bldr_plugin.ExPluginLoadAccessClient(
		...
		func(ctx context.Context, cc srpc.Client) error {
			...
			call, err := client.HandleWebPkgsViaPluginAssets(ctx, c.conf.ToRequest())
			if err != nil {
				return err
			}
			for {
				...
				case *bldr_web_plugin.HandleWebPkgsViaPluginAssetsResponse_Ready:
					if b.Ready {
						c.le.Debugf("web plugin: serving web pkgs from plugin %s assets is ready", c.conf.GetHandlePluginId())
					} else {
						c.le.Debugf("web plugin: serving web pkgs from plugin %s assets is not ready", c.conf.GetHandlePluginId())
					}
				}
			}
		},
