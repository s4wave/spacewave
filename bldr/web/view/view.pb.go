...
func (m *SetHtmlLinksRequest) CloneVT() *SetHtmlLinksRequest {
	...
	if rhs := m.Remove; rhs != nil {
		r.Remove = slices.Clone(rhs)
	}
	if rhs := m.SetLinks; rhs != nil {
		r.SetLinks = make(map[string]*HtmlLink, len(rhs))
		for k, v := range rhs {
			r.SetLinks[k] = v.CloneVT()
		}
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
...
func (x *SetHtmlLinksRequest) MarshalProtoText() string {
	...
	if len(x.SetLinks) > 0 {
		...
		for _, k := range slices.Sorted(maps.Keys(x.SetLinks)) {
			v := x.SetLinks[k]
			sb.WriteString(" ")
			sb.WriteString(strconv.Quote(k))
			sb.WriteString(": ")
			if v == nil {
				sb.WriteString((&HtmlLink{}).MarshalProtoText())
			} else {
