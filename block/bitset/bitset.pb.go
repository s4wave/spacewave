...
func (m *BitSet) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return errors.Errorf("proto: BitSet: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: BitSet: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType == 0 {
				...
				return errors.Errorf("proto: wrong wireType = %d for field Set", wireType)
			}
		case 2:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field Len", wireType)
			}
			m.Len = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
