...
func (m *BloomFilter) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return errors.Errorf("proto: BloomFilter: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: BloomFilter: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field K", wireType)
			}
			m.K = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
		...
		case 2:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field M", wireType)
			}
			m.M = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
		...
		case 3:
			if wireType != 2 {
				return errors.Errorf("proto: wrong wireType = %d for field BitSet", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
