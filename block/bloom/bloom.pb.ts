/* eslint-disable */
import Long from "long";
import _m0 from "protobufjs/minimal.js";
import { BitSet } from "../bitset/bitset.pb.js";

export const protobufPackage = "bloom";

/**
 * BloomFilter contains a bloom filter in a Block.
 * uses FNV64 hash function
 * m factor is determined by len(bit_set)*64
 */
export interface BloomFilter {
  /**
   * K is the number of hash functions.
   * Max: 10.
   * Min: 1.
   */
  k: number;
  /** M is the length (capacity) of the bloom filter. */
  m: number;
  /** BitSet is the underlying bitset. */
  bitSet: BitSet | undefined;
}

function createBaseBloomFilter(): BloomFilter {
  return { k: 0, m: 0, bitSet: undefined };
}

export const BloomFilter = {
  encode(message: BloomFilter, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.k !== 0) {
      writer.uint32(8).uint32(message.k);
    }
    if (message.m !== 0) {
      writer.uint32(16).uint32(message.m);
    }
    if (message.bitSet !== undefined) {
      BitSet.encode(message.bitSet, writer.uint32(26).fork()).ldelim();
    }
    return writer;
  },

  decode(input: _m0.Reader | Uint8Array, length?: number): BloomFilter {
    const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseBloomFilter();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
          message.k = reader.uint32();
          break;
        case 2:
          message.m = reader.uint32();
          break;
        case 3:
          message.bitSet = BitSet.decode(reader, reader.uint32());
          break;
        default:
          reader.skipType(tag & 7);
          break;
      }
    }
    return message;
  },

  // encodeTransform encodes a source of message objects.
  // Transform<BloomFilter, Uint8Array>
  async *encodeTransform(
    source: AsyncIterable<BloomFilter | BloomFilter[]> | Iterable<BloomFilter | BloomFilter[]>,
  ): AsyncIterable<Uint8Array> {
    for await (const pkt of source) {
      if (Array.isArray(pkt)) {
        for (const p of pkt) {
          yield* [BloomFilter.encode(p).finish()];
        }
      } else {
        yield* [BloomFilter.encode(pkt).finish()];
      }
    }
  },

  // decodeTransform decodes a source of encoded messages.
  // Transform<Uint8Array, BloomFilter>
  async *decodeTransform(
    source: AsyncIterable<Uint8Array | Uint8Array[]> | Iterable<Uint8Array | Uint8Array[]>,
  ): AsyncIterable<BloomFilter> {
    for await (const pkt of source) {
      if (Array.isArray(pkt)) {
        for (const p of pkt) {
          yield* [BloomFilter.decode(p)];
        }
      } else {
        yield* [BloomFilter.decode(pkt)];
      }
    }
  },

  fromJSON(object: any): BloomFilter {
    return {
      k: isSet(object.k) ? Number(object.k) : 0,
      m: isSet(object.m) ? Number(object.m) : 0,
      bitSet: isSet(object.bitSet) ? BitSet.fromJSON(object.bitSet) : undefined,
    };
  },

  toJSON(message: BloomFilter): unknown {
    const obj: any = {};
    message.k !== undefined && (obj.k = Math.round(message.k));
    message.m !== undefined && (obj.m = Math.round(message.m));
    message.bitSet !== undefined && (obj.bitSet = message.bitSet ? BitSet.toJSON(message.bitSet) : undefined);
    return obj;
  },

  fromPartial<I extends Exact<DeepPartial<BloomFilter>, I>>(object: I): BloomFilter {
    const message = createBaseBloomFilter();
    message.k = object.k ?? 0;
    message.m = object.m ?? 0;
    message.bitSet = (object.bitSet !== undefined && object.bitSet !== null)
      ? BitSet.fromPartial(object.bitSet)
      : undefined;
    return message;
  },
};

type Builtin = Date | Function | Uint8Array | string | number | boolean | undefined;

export type DeepPartial<T> = T extends Builtin ? T
  : T extends Long ? string | number | Long : T extends Array<infer U> ? Array<DeepPartial<U>>
  : T extends ReadonlyArray<infer U> ? ReadonlyArray<DeepPartial<U>>
  : T extends { $case: string } ? { [K in keyof Omit<T, "$case">]?: DeepPartial<T[K]> } & { $case: T["$case"] }
  : T extends {} ? { [K in keyof T]?: DeepPartial<T[K]> }
  : Partial<T>;

type KeysOfUnion<T> = T extends T ? keyof T : never;
export type Exact<P, I extends P> = P extends Builtin ? P
  : P & { [K in keyof P]: Exact<P[K], I[K]> } & { [K in Exclude<keyof I, KeysOfUnion<P>>]: never };

if (_m0.util.Long !== Long) {
  _m0.util.Long = Long as any;
  _m0.configure();
}

function isSet(value: any): boolean {
  return value !== null && value !== undefined;
}
