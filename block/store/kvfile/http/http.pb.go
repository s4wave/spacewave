...
func (m *Config) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field DisableCache", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
		...
		case 8:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field MinRequestSize", wireType)
			}
			m.MinRequestSize = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
