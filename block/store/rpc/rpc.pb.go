...
func (m *Config) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return errors.Errorf("proto: wrong wireType = %d for field BlockStoreIds", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
		...
		case 8:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field LookupOnStart", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
