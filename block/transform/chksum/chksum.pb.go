...
func (x ChksumType) MarshalProtoJSON(s *json.MarshalState) {
	s.WriteEnumString(int32(x), ChksumType_name)
}
...
func (m *Config) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field ChksumType", wireType)
			}
			m.ChksumType = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.ChksumType |= ChksumType(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
