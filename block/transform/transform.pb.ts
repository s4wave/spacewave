...
export const Config = {
  ...
  fromJSON(object: any): Config {
    return { steps: Array.isArray(object?.steps) ? object.steps.map((e: any) => StepConfig.fromJSON(e)) : [] };
  },

  toJSON(message: Config): unknown {
    const obj: any = {};
    if (message.steps) {
      obj.steps = message.steps.map((e) => e ? StepConfig.toJSON(e) : undefined);
    } else {
      obj.steps = [];
    }
    return obj;
...
export const StepConfig = {
  encode(message: StepConfig, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.id !== "") {
  ...
  async *encodeTransform(
    source: AsyncIterable<StepConfig | StepConfig[]> | Iterable<StepConfig | StepConfig[]>,
  ): AsyncIterable<Uint8Array> {
    for await (const pkt of source) {
      if (Array.isArray(pkt)) {
        for (const p of pkt) {
  ...
  fromPartial<I extends Exact<DeepPartial<StepConfig>, I>>(object: I): StepConfig {
    const message = createBaseStepConfig();
