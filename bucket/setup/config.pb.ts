...
export const Config = {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): Config {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.applyBucketConfigs.push(ApplyBucketConfig.decode(reader, reader.uint32()));
          break;
        default:
  ...
  fromJSON(object: any): Config {
    return {
      applyBucketConfigs: Array.isArray(object?.applyBucketConfigs)
        ? object.applyBucketConfigs.map((e: any) => ApplyBucketConfig.fromJSON(e))
        : [],
    };
  },
  ...
  toJSON(message: Config): unknown {
    ...
    if (message.applyBucketConfigs) {
      obj.applyBucketConfigs = message.applyBucketConfigs.map((e) => e ? ApplyBucketConfig.toJSON(e) : undefined);
    } else {
  ...
  fromPartial<I extends Exact<DeepPartial<Config>, I>>(object: I): Config {
    const message = createBaseConfig();
    message.applyBucketConfigs = object.applyBucketConfigs?.map((e) => ApplyBucketConfig.fromPartial(e)) || [];
    return message;
  },
};
...
export const ApplyBucketConfig = {
  encode(message: ApplyBucketConfig, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
  ...
  async *encodeTransform(
    source: AsyncIterable<ApplyBucketConfig | ApplyBucketConfig[]> | Iterable<ApplyBucketConfig | ApplyBucketConfig[]>,
  ): AsyncIterable<Uint8Array> {
    for await (const pkt of source) {
      if (Array.isArray(pkt)) {
        for (const p of pkt) {
  ...
  fromPartial<I extends Exact<DeepPartial<ApplyBucketConfig>, I>>(object: I): ApplyBucketConfig {
    const message = createBaseApplyBucketConfig();
