...
// newComposedBus builds the distribution core bus with the Spacewave
// composition, as the generated distribution entrypoint does.
func newComposedBus(ctx context.Context, le *logrus.Entry) (bus.Bus, error) {
	b, sr, err := dist_entrypoint.NewCoreBus(ctx, le)
	if err != nil {
		return nil, err
	}
	Compose().AddFactories(b, sr)
	return b, nil
}
