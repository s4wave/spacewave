...
func (m *CreateGitRepoWizardOp) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.Clone {
		i = protobuf_go_lite.EncodeBool(dAtA, i, m.Clone)
		i--
		dAtA[i] = 0x18
	}
	if len(m.Name) > 0 {
...
func (m *CreateGitRepoWizardOp) SizeVT() (n int) {
	...
	n += protobuf_go_lite.SizeBoolNonZero(1, m.Clone)
	if m.CloneOpts != nil {
...
func (x *CreateGitRepoWizardOp) MarshalProtoText() string {
	...
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "CreateGitRepoWizardOp")
	if x.ObjectKey != "" {
	...
	if x.Clone != false {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "clone")
		protobuf_go_lite.TextWriteBool(&sb, x.Clone)
	}
	if x.CloneOpts != nil {
