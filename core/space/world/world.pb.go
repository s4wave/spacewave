// Code generated by protoc-gen-go-lite. DO NOT EDIT.
// protoc-gen-go-lite version: v0.19.0
// source: github.com/s4wave/spacewave/core/space/world/world.proto

package space_world

import (
	fmt "fmt"
	io "io"
	slices "slices"

	protobuf_go_lite "github.com/aperturerobotics/protobuf-go-lite"
	json "github.com/aperturerobotics/protobuf-go-lite/json"
	command "github.com/s4wave/spacewave/sdk/command"
)

// WorldContents contains a list of world objects and types.
type WorldContents struct {
	unknownFields []byte
	// Objects contains the list of world objects.
	Objects []*WorldContentsObject `protobuf:"bytes,1,rep,name=objects,proto3" json:"objects,omitempty"`
	// ObjectTypes contains the list of world object types.
	ObjectTypes []*WorldContentsObjectType `protobuf:"bytes,2,rep,name=object_types,json=objectTypes,proto3" json:"objectTypes,omitempty"`
}

func (x *WorldContents) Reset() {
	*x = WorldContents{}
}

func (*WorldContents) ProtoMessage() {}

func (x *WorldContents) GetObjects() []*WorldContentsObject {
	if x != nil {
		return x.Objects
	}
	return nil
}

func (x *WorldContents) GetObjectTypes() []*WorldContentsObjectType {
	if x != nil {
		return x.ObjectTypes
	}
	return nil
}

// WorldContentsObject is an entry in the WorldContents.
type WorldContentsObject struct {
	unknownFields []byte
	// ObjectKey is the key of the object in the world.
	ObjectKey string `protobuf:"bytes,1,opt,name=object_key,json=objectKey,proto3" json:"objectKey,omitempty"`
	// ParentObjectKey is the key of the parent of the object.
	// Can be empty.
	ParentObjectKey string `protobuf:"bytes,2,opt,name=parent_object_key,json=parentObjectKey,proto3" json:"parentObjectKey,omitempty"`
	// ObjectType is the type of the object.
	ObjectType string `protobuf:"bytes,3,opt,name=object_type,json=objectType,proto3" json:"objectType,omitempty"`
}

func (x *WorldContentsObject) Reset() {
	*x = WorldContentsObject{}
}

func (*WorldContentsObject) ProtoMessage() {}

func (x *WorldContentsObject) GetObjectKey() string {
	if x != nil {
		return x.ObjectKey
	}
	return ""
}

func (x *WorldContentsObject) GetParentObjectKey() string {
	if x != nil {
		return x.ParentObjectKey
	}
	return ""
}

func (x *WorldContentsObject) GetObjectType() string {
	if x != nil {
		return x.ObjectType
	}
	return ""
}

// WorldContentsObjectType is an entry in the WorldContents.
type WorldContentsObjectType struct {
	unknownFields []byte
	// ObjectType is the type id.
	ObjectType string `protobuf:"bytes,1,opt,name=object_type,json=objectType,proto3" json:"objectType,omitempty"`
}

func (x *WorldContentsObjectType) Reset() {
	*x = WorldContentsObjectType{}
}

func (*WorldContentsObjectType) ProtoMessage() {}

func (x *WorldContentsObjectType) GetObjectType() string {
	if x != nil {
		return x.ObjectType
	}
	return ""
}

// SpaceSettings is an object containing the settings for a Space.
//
// Note: the space name and other "public" details are in the SpaceSoMeta.
type SpaceSettings struct {
	unknownFields []byte
	// IndexPath is the default URL path to display at /.
	IndexPath string `protobuf:"bytes,1,opt,name=index_path,json=indexPath,proto3" json:"indexPath,omitempty"`
	// PluginIds lists manifest IDs of plugins this Space uses.
	// Shared with all Space participants.
	PluginIds []string `protobuf:"bytes,2,rep,name=plugin_ids,json=pluginIds,proto3" json:"pluginIds,omitempty"`
	// KeybindingOverrides stores Space-scope keybinding overrides.
	KeybindingOverrides *command.KeybindingOverrideSet `protobuf:"bytes,3,opt,name=keybinding_overrides,json=keybindingOverrides,proto3" json:"keybindingOverrides,omitempty"`
	// PluginInstallations pins plugins to immutable World artifacts.
	// Plugins without an installation follow their configured application catalog.
	PluginInstallations map[string]*SpacePluginInstallation `protobuf:"bytes,4,rep,name=plugin_installations,json=pluginInstallations,proto3" json:"pluginInstallations,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}

func (x *SpaceSettings) Reset() {
	*x = SpaceSettings{}
}

func (*SpaceSettings) ProtoMessage() {}

func (x *SpaceSettings) GetIndexPath() string {
	if x != nil {
		return x.IndexPath
	}
	return ""
}

func (x *SpaceSettings) GetPluginIds() []string {
	if x != nil {
		return x.PluginIds
	}
	return nil
}

func (x *SpaceSettings) GetKeybindingOverrides() *command.KeybindingOverrideSet {
	if x != nil {
		return x.KeybindingOverrides
	}
	return nil
}

func (x *SpaceSettings) GetPluginInstallations() map[string]*SpacePluginInstallation {
	if x != nil {
		return x.PluginInstallations
	}
	return nil
}

// SpacePluginInstallation retains explicit installation choices across reopen.
type SpacePluginInstallation struct {
	unknownFields []byte
	// ManifestKeys lists immutable artifacts, newest installation first.
	// Earlier choices recover a working plugin when the requested version fails.
	ManifestKeys []string `protobuf:"bytes,1,rep,name=manifest_keys,json=manifestKeys,proto3" json:"manifestKeys,omitempty"`
}

func (x *SpacePluginInstallation) Reset() {
	*x = SpacePluginInstallation{}
}

func (*SpacePluginInstallation) ProtoMessage() {}

func (x *SpacePluginInstallation) GetManifestKeys() []string {
	if x != nil {
		return x.ManifestKeys
	}
	return nil
}

type SpaceSettings_PluginInstallationsEntry struct {
	unknownFields []byte
	Key           string                   `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	Value         *SpacePluginInstallation `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"`
}

func (x *SpaceSettings_PluginInstallationsEntry) Reset() {
	*x = SpaceSettings_PluginInstallationsEntry{}
}

func (*SpaceSettings_PluginInstallationsEntry) ProtoMessage() {}

func (x *SpaceSettings_PluginInstallationsEntry) GetKey() string {
	if x != nil {
		return x.Key
	}
	return ""
}

func (x *SpaceSettings_PluginInstallationsEntry) GetValue() *SpacePluginInstallation {
	if x != nil {
		return x.Value
	}
	return nil
}

func (m *WorldContents) CloneVT() *WorldContents {
	if m == nil {
		return (*WorldContents)(nil)
	}
	r := new(WorldContents)
	r.Objects = protobuf_go_lite.CloneVTSlice(m.Objects)
	r.ObjectTypes = protobuf_go_lite.CloneVTSlice(m.ObjectTypes)
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *WorldContents) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (m *WorldContentsObject) CloneVT() *WorldContentsObject {
	if m == nil {
		return (*WorldContentsObject)(nil)
	}
	r := new(WorldContentsObject)
	r.ObjectKey = m.ObjectKey
	r.ParentObjectKey = m.ParentObjectKey
	r.ObjectType = m.ObjectType
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *WorldContentsObject) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (m *WorldContentsObjectType) CloneVT() *WorldContentsObjectType {
	if m == nil {
		return (*WorldContentsObjectType)(nil)
	}
	r := new(WorldContentsObjectType)
	r.ObjectType = m.ObjectType
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *WorldContentsObjectType) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (m *SpaceSettings) CloneVT() *SpaceSettings {
	if m == nil {
		return (*SpaceSettings)(nil)
	}
	r := new(SpaceSettings)
	r.IndexPath = m.IndexPath
	r.PluginIds = protobuf_go_lite.CloneSlice(m.PluginIds)
	r.KeybindingOverrides = protobuf_go_lite.CloneVTValue(m.KeybindingOverrides)
	r.PluginInstallations = protobuf_go_lite.CloneVTMap(m.PluginInstallations)
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *SpaceSettings) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (m *SpacePluginInstallation) CloneVT() *SpacePluginInstallation {
	if m == nil {
		return (*SpacePluginInstallation)(nil)
	}
	r := new(SpacePluginInstallation)
	r.ManifestKeys = protobuf_go_lite.CloneSlice(m.ManifestKeys)
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *SpacePluginInstallation) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (this *WorldContents) EqualVT(that *WorldContents) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if !protobuf_go_lite.EqualVTSliceImplicit(this.Objects, that.Objects, func() *WorldContentsObject { return &WorldContentsObject{} }) {
		return false
	}
	if !protobuf_go_lite.EqualVTSliceImplicit(this.ObjectTypes, that.ObjectTypes, func() *WorldContentsObjectType { return &WorldContentsObjectType{} }) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *WorldContents) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*WorldContents)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

func (this *WorldContentsObject) EqualVT(that *WorldContentsObject) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if this.ObjectKey != that.ObjectKey {
		return false
	}
	if this.ParentObjectKey != that.ParentObjectKey {
		return false
	}
	if this.ObjectType != that.ObjectType {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *WorldContentsObject) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*WorldContentsObject)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

func (this *WorldContentsObjectType) EqualVT(that *WorldContentsObjectType) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if this.ObjectType != that.ObjectType {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *WorldContentsObjectType) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*WorldContentsObjectType)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

func (this *SpaceSettings) EqualVT(that *SpaceSettings) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if this.IndexPath != that.IndexPath {
		return false
	}
	if !protobuf_go_lite.EqualSlice(this.PluginIds, that.PluginIds) {
		return false
	}
	if !protobuf_go_lite.IsEqualVT(this.KeybindingOverrides, that.KeybindingOverrides) {
		return false
	}
	if !protobuf_go_lite.EqualVTMapImplicit(this.PluginInstallations, that.PluginInstallations, func() *SpacePluginInstallation { return &SpacePluginInstallation{} }) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *SpaceSettings) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*SpaceSettings)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

func (this *SpacePluginInstallation) EqualVT(that *SpacePluginInstallation) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if !protobuf_go_lite.EqualSlice(this.ManifestKeys, that.ManifestKeys) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *SpacePluginInstallation) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*SpacePluginInstallation)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

// MarshalProtoJSON marshals the WorldContents message to JSON.
func (x *WorldContents) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if len(x.Objects) > 0 || s.HasField("objects") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("objects")
		s.WriteArrayStart()
		var wroteElement bool
		for _, element := range x.Objects {
			s.WriteMoreIf(&wroteElement)
			element.MarshalProtoJSON(s.WithField("objects"))
		}
		s.WriteArrayEnd()
	}
	if len(x.ObjectTypes) > 0 || s.HasField("objectTypes") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("objectTypes")
		s.WriteArrayStart()
		var wroteElement bool
		for _, element := range x.ObjectTypes {
			s.WriteMoreIf(&wroteElement)
			element.MarshalProtoJSON(s.WithField("objectTypes"))
		}
		s.WriteArrayEnd()
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the WorldContents to JSON.
func (x *WorldContents) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the WorldContents message from JSON.
func (x *WorldContents) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "objects":
			s.AddField("objects")
			if s.ReadNil() {
				x.Objects = nil
				return
			}
			s.ReadArray(func() {
				if s.ReadNil() {
					x.Objects = append(x.Objects, nil)
					return
				}
				v := &WorldContentsObject{}
				v.UnmarshalProtoJSON(s.WithField("objects", false))
				if s.Err() != nil {
					return
				}
				x.Objects = append(x.Objects, v)
			})
		case "object_types", "objectTypes":
			s.AddField("object_types")
			if s.ReadNil() {
				x.ObjectTypes = nil
				return
			}
			s.ReadArray(func() {
				if s.ReadNil() {
					x.ObjectTypes = append(x.ObjectTypes, nil)
					return
				}
				v := &WorldContentsObjectType{}
				v.UnmarshalProtoJSON(s.WithField("object_types", false))
				if s.Err() != nil {
					return
				}
				x.ObjectTypes = append(x.ObjectTypes, v)
			})
		}
	})
}

// UnmarshalJSON unmarshals the WorldContents from JSON.
func (x *WorldContents) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the WorldContentsObject message to JSON.
func (x *WorldContentsObject) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if x.ObjectKey != "" || s.HasField("objectKey") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("objectKey")
		s.WriteString(x.ObjectKey)
	}
	if x.ParentObjectKey != "" || s.HasField("parentObjectKey") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("parentObjectKey")
		s.WriteString(x.ParentObjectKey)
	}
	if x.ObjectType != "" || s.HasField("objectType") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("objectType")
		s.WriteString(x.ObjectType)
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the WorldContentsObject to JSON.
func (x *WorldContentsObject) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the WorldContentsObject message from JSON.
func (x *WorldContentsObject) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "object_key", "objectKey":
			s.AddField("object_key")
			x.ObjectKey = s.ReadString()
		case "parent_object_key", "parentObjectKey":
			s.AddField("parent_object_key")
			x.ParentObjectKey = s.ReadString()
		case "object_type", "objectType":
			s.AddField("object_type")
			x.ObjectType = s.ReadString()
		}
	})
}

// UnmarshalJSON unmarshals the WorldContentsObject from JSON.
func (x *WorldContentsObject) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the WorldContentsObjectType message to JSON.
func (x *WorldContentsObjectType) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if x.ObjectType != "" || s.HasField("objectType") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("objectType")
		s.WriteString(x.ObjectType)
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the WorldContentsObjectType to JSON.
func (x *WorldContentsObjectType) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the WorldContentsObjectType message from JSON.
func (x *WorldContentsObjectType) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "object_type", "objectType":
			s.AddField("object_type")
			x.ObjectType = s.ReadString()
		}
	})
}

// UnmarshalJSON unmarshals the WorldContentsObjectType from JSON.
func (x *WorldContentsObjectType) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the SpaceSettings_PluginInstallationsEntry message to JSON.
func (x *SpaceSettings_PluginInstallationsEntry) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if x.Key != "" || s.HasField("key") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("key")
		s.WriteString(x.Key)
	}
	if x.Value != nil || s.HasField("value") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("value")
		x.Value.MarshalProtoJSON(s.WithField("value"))
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the SpaceSettings_PluginInstallationsEntry to JSON.
func (x *SpaceSettings_PluginInstallationsEntry) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the SpaceSettings_PluginInstallationsEntry message from JSON.
func (x *SpaceSettings_PluginInstallationsEntry) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "key":
			s.AddField("key")
			x.Key = s.ReadString()
		case "value":
			if s.ReadNil() {
				x.Value = nil
				return
			}
			x.Value = &SpacePluginInstallation{}
			x.Value.UnmarshalProtoJSON(s.WithField("value", true))
		}
	})
}

// UnmarshalJSON unmarshals the SpaceSettings_PluginInstallationsEntry from JSON.
func (x *SpaceSettings_PluginInstallationsEntry) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the SpaceSettings message to JSON.
func (x *SpaceSettings) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if x.IndexPath != "" || s.HasField("indexPath") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("indexPath")
		s.WriteString(x.IndexPath)
	}
	if len(x.PluginIds) > 0 || s.HasField("pluginIds") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("pluginIds")
		s.WriteStringArray(x.PluginIds)
	}
	if x.KeybindingOverrides != nil || s.HasField("keybindingOverrides") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("keybindingOverrides")
		x.KeybindingOverrides.MarshalProtoJSON(s.WithField("keybindingOverrides"))
	}
	if x.PluginInstallations != nil || s.HasField("pluginInstallations") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("pluginInstallations")
		s.WriteObjectStart()
		var wroteElement bool
		for k, v := range x.PluginInstallations {
			s.WriteMoreIf(&wroteElement)
			s.WriteObjectStringField(k)
			v.MarshalProtoJSON(s.WithField("pluginInstallations"))
		}
		s.WriteObjectEnd()
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the SpaceSettings to JSON.
func (x *SpaceSettings) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the SpaceSettings message from JSON.
func (x *SpaceSettings) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "index_path", "indexPath":
			s.AddField("index_path")
			x.IndexPath = s.ReadString()
		case "plugin_ids", "pluginIds":
			s.AddField("plugin_ids")
			if s.ReadNil() {
				x.PluginIds = nil
				return
			}
			x.PluginIds = s.ReadStringArray()
		case "keybinding_overrides", "keybindingOverrides":
			if s.ReadNil() {
				x.KeybindingOverrides = nil
				return
			}
			x.KeybindingOverrides = &command.KeybindingOverrideSet{}
			x.KeybindingOverrides.UnmarshalProtoJSON(s.WithField("keybinding_overrides", true))
		case "plugin_installations", "pluginInstallations":
			s.AddField("plugin_installations")
			if s.ReadNil() {
				x.PluginInstallations = nil
				return
			}
			x.PluginInstallations = make(map[string]*SpacePluginInstallation)
			s.ReadStringMap(func(key string) {
				var v SpacePluginInstallation
				v.UnmarshalProtoJSON(s)
				x.PluginInstallations[key] = &v
			})
		}
	})
}

// UnmarshalJSON unmarshals the SpaceSettings from JSON.
func (x *SpaceSettings) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the SpacePluginInstallation message to JSON.
func (x *SpacePluginInstallation) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if len(x.ManifestKeys) > 0 || s.HasField("manifestKeys") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("manifestKeys")
		s.WriteStringArray(x.ManifestKeys)
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the SpacePluginInstallation to JSON.
func (x *SpacePluginInstallation) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the SpacePluginInstallation message from JSON.
func (x *SpacePluginInstallation) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "manifest_keys", "manifestKeys":
			s.AddField("manifest_keys")
			if s.ReadNil() {
				x.ManifestKeys = nil
				return
			}
			x.ManifestKeys = s.ReadStringArray()
		}
	})
}

// UnmarshalJSON unmarshals the SpacePluginInstallation from JSON.
func (x *SpacePluginInstallation) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

func (m *WorldContents) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *WorldContents) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *WorldContents) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	if len(m.ObjectTypes) > 0 {
		for iNdEx := len(m.ObjectTypes) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.ObjectTypes[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
			i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
		}
	}
	if len(m.Objects) > 0 {
		for iNdEx := len(m.Objects) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.Objects[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
			i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0xa
		}
	}
	return len(dAtA) - i, nil
}

func (m *WorldContentsObject) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *WorldContentsObject) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *WorldContentsObject) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	if len(m.ObjectType) > 0 {
		i = protobuf_go_lite.EncodeString(dAtA, i, m.ObjectType)
		i--
		dAtA[i] = 0x1a
	}
	if len(m.ParentObjectKey) > 0 {
		i = protobuf_go_lite.EncodeString(dAtA, i, m.ParentObjectKey)
		i--
		dAtA[i] = 0x12
	}
	if len(m.ObjectKey) > 0 {
		i = protobuf_go_lite.EncodeString(dAtA, i, m.ObjectKey)
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *WorldContentsObjectType) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *WorldContentsObjectType) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *WorldContentsObjectType) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	if len(m.ObjectType) > 0 {
		i = protobuf_go_lite.EncodeString(dAtA, i, m.ObjectType)
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *SpaceSettings) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *SpaceSettings) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *SpaceSettings) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	if len(m.PluginInstallations) > 0 {
		for k := range m.PluginInstallations {
			v := m.PluginInstallations[k]
			baseI := i
			size, err := v.MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
			i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
			i = protobuf_go_lite.EncodeString(dAtA, i, k)
			i--
			dAtA[i] = 0xa
			i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(baseI-i))
			i--
			dAtA[i] = 0x22
		}
	}
	if m.KeybindingOverrides != nil {
		size, err := m.KeybindingOverrides.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.PluginIds) > 0 {
		for iNdEx := len(m.PluginIds) - 1; iNdEx >= 0; iNdEx-- {
			i = protobuf_go_lite.EncodeString(dAtA, i, m.PluginIds[iNdEx])
			i--
			dAtA[i] = 0x12
		}
	}
	if len(m.IndexPath) > 0 {
		i = protobuf_go_lite.EncodeString(dAtA, i, m.IndexPath)
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *SpacePluginInstallation) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *SpacePluginInstallation) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *SpacePluginInstallation) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	if len(m.ManifestKeys) > 0 {
		for iNdEx := len(m.ManifestKeys) - 1; iNdEx >= 0; iNdEx-- {
			i = protobuf_go_lite.EncodeString(dAtA, i, m.ManifestKeys[iNdEx])
			i--
			dAtA[i] = 0xa
		}
	}
	return len(dAtA) - i, nil
}

func (m *WorldContents) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	for _, e := range m.Objects {
		l = e.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	for _, e := range m.ObjectTypes {
		l = e.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	n += len(m.unknownFields)
	return n
}

func (m *WorldContentsObject) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	n += protobuf_go_lite.SizeStringNonEmpty(1, m.ObjectKey)
	n += protobuf_go_lite.SizeStringNonEmpty(1, m.ParentObjectKey)
	n += protobuf_go_lite.SizeStringNonEmpty(1, m.ObjectType)
	n += len(m.unknownFields)
	return n
}

func (m *WorldContentsObjectType) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	n += protobuf_go_lite.SizeStringNonEmpty(1, m.ObjectType)
	n += len(m.unknownFields)
	return n
}

func (m *SpaceSettings) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	n += protobuf_go_lite.SizeStringNonEmpty(1, m.IndexPath)
	n += protobuf_go_lite.SizeStringSlice(1, m.PluginIds)
	if m.KeybindingOverrides != nil {
		l = m.KeybindingOverrides.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	for k, v := range m.PluginInstallations {
		_ = k
		_ = v
		l = 0
		if v != nil {
			l = v.SizeVT()
		}
		mapEntrySize := protobuf_go_lite.SizeStringValue(1, k) + protobuf_go_lite.SizeMessage(1, l)
		n += protobuf_go_lite.SizeMessage(1, mapEntrySize)
	}
	n += len(m.unknownFields)
	return n
}

func (m *SpacePluginInstallation) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	n += protobuf_go_lite.SizeStringSlice(1, m.ManifestKeys)
	n += len(m.unknownFields)
	return n
}

func (x *WorldContents) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "WorldContents")
	if len(x.Objects) > 0 {
		protobuf_go_lite.TextWriteListStart(&sb, initialLen, "objects")
		for i, v := range x.Objects {
			protobuf_go_lite.TextWriteListSeparator(&sb, i)
			if v == nil {
				protobuf_go_lite.TextWriteTextMarshaler(&sb, &WorldContentsObject{})
			} else {
				protobuf_go_lite.TextWriteTextMarshaler(&sb, v)
			}
		}
		protobuf_go_lite.TextWriteListEnd(&sb)
	}
	if len(x.ObjectTypes) > 0 {
		protobuf_go_lite.TextWriteListStart(&sb, initialLen, "object_types")
		for i, v := range x.ObjectTypes {
			protobuf_go_lite.TextWriteListSeparator(&sb, i)
			if v == nil {
				protobuf_go_lite.TextWriteTextMarshaler(&sb, &WorldContentsObjectType{})
			} else {
				protobuf_go_lite.TextWriteTextMarshaler(&sb, v)
			}
		}
		protobuf_go_lite.TextWriteListEnd(&sb)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *WorldContents) String() string {
	return x.MarshalProtoText()
}

func (x *WorldContentsObject) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "WorldContentsObject")
	if x.ObjectKey != "" {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "object_key")
		protobuf_go_lite.TextWriteString(&sb, x.ObjectKey)
	}
	if x.ParentObjectKey != "" {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "parent_object_key")
		protobuf_go_lite.TextWriteString(&sb, x.ParentObjectKey)
	}
	if x.ObjectType != "" {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "object_type")
		protobuf_go_lite.TextWriteString(&sb, x.ObjectType)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *WorldContentsObject) String() string {
	return x.MarshalProtoText()
}

func (x *WorldContentsObjectType) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "WorldContentsObjectType")
	if x.ObjectType != "" {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "object_type")
		protobuf_go_lite.TextWriteString(&sb, x.ObjectType)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *WorldContentsObjectType) String() string {
	return x.MarshalProtoText()
}

func (x *SpaceSettings_PluginInstallationsEntry) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "PluginInstallationsEntry")
	if x.Key != "" {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "key")
		protobuf_go_lite.TextWriteString(&sb, x.Key)
	}
	if x.Value != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "value")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.Value)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *SpaceSettings_PluginInstallationsEntry) String() string {
	return x.MarshalProtoText()
}

func (x *SpaceSettings) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "SpaceSettings")
	if x.IndexPath != "" {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "index_path")
		protobuf_go_lite.TextWriteString(&sb, x.IndexPath)
	}
	if len(x.PluginIds) > 0 {
		protobuf_go_lite.TextWriteListStart(&sb, initialLen, "plugin_ids")
		for i, v := range x.PluginIds {
			protobuf_go_lite.TextWriteListSeparator(&sb, i)
			protobuf_go_lite.TextWriteString(&sb, v)
		}
		protobuf_go_lite.TextWriteListEnd(&sb)
	}
	if x.KeybindingOverrides != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "keybinding_overrides")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.KeybindingOverrides)
	}
	if len(x.PluginInstallations) > 0 {
		protobuf_go_lite.TextWriteMapStart(&sb, initialLen, "plugin_installations")
		for _, k := range protobuf_go_lite.TextSortedMapKeys(x.PluginInstallations) {
			v := x.PluginInstallations[k]
			protobuf_go_lite.TextWriteMapEntryPrefix(&sb)
			protobuf_go_lite.TextWriteString(&sb, k)
			protobuf_go_lite.TextWriteMapKeyValueSeparator(&sb)
			if v == nil {
				protobuf_go_lite.TextWriteTextMarshaler(&sb, &SpacePluginInstallation{})
			} else {
				protobuf_go_lite.TextWriteTextMarshaler(&sb, v)
			}
		}
		protobuf_go_lite.TextWriteMapEnd(&sb)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *SpaceSettings) String() string {
	return x.MarshalProtoText()
}

func (x *SpacePluginInstallation) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "SpacePluginInstallation")
	if len(x.ManifestKeys) > 0 {
		protobuf_go_lite.TextWriteListStart(&sb, initialLen, "manifest_keys")
		for i, v := range x.ManifestKeys {
			protobuf_go_lite.TextWriteListSeparator(&sb, i)
			protobuf_go_lite.TextWriteString(&sb, v)
		}
		protobuf_go_lite.TextWriteListEnd(&sb)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *SpacePluginInstallation) String() string {
	return x.MarshalProtoText()
}

func (m *WorldContents) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: WorldContents: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: WorldContents: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Objects", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.Objects = append(m.Objects, &WorldContentsObject{})
			if err := m.Objects[len(m.Objects)-1].UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ObjectTypes", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.ObjectTypes = append(m.ObjectTypes, &WorldContentsObjectType{})
			if err := m.ObjectTypes[len(m.ObjectTypes)-1].UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}

func (m *WorldContentsObject) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: WorldContentsObject: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: WorldContentsObject: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ObjectKey", wireType)
			}
			var v string
			v, iNdEx, err = protobuf_go_lite.DecodeString(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.ObjectKey = v
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ParentObjectKey", wireType)
			}
			var v string
			v, iNdEx, err = protobuf_go_lite.DecodeString(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.ParentObjectKey = v
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ObjectType", wireType)
			}
			var v string
			v, iNdEx, err = protobuf_go_lite.DecodeString(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.ObjectType = v
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}

func (m *WorldContentsObjectType) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: WorldContentsObjectType: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: WorldContentsObjectType: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ObjectType", wireType)
			}
			var v string
			v, iNdEx, err = protobuf_go_lite.DecodeString(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.ObjectType = v
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}

func (m *SpaceSettings) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: SpaceSettings: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: SpaceSettings: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field IndexPath", wireType)
			}
			var v string
			v, iNdEx, err = protobuf_go_lite.DecodeString(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.IndexPath = v
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PluginIds", wireType)
			}
			var v string
			v, iNdEx, err = protobuf_go_lite.DecodeString(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.PluginIds = append(m.PluginIds, v)
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field KeybindingOverrides", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			if m.KeybindingOverrides == nil {
				m.KeybindingOverrides = &command.KeybindingOverrideSet{}
			}
			if err := m.KeybindingOverrides.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PluginInstallations", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			iNdEx = msgStart
			if m.PluginInstallations == nil {
				m.PluginInstallations = make(map[string]*SpacePluginInstallation)
			}
			var mapkey string
			var mapvalue *SpacePluginInstallation
			for iNdEx < postIndex {
				entryPreIndex := iNdEx
				var wire uint64
				wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
				if err != nil {
					return err
				}
				fieldNum := int32(wire >> 3)
				if fieldNum == 1 {
					mapkey, iNdEx, err = protobuf_go_lite.DecodeString(dAtA, iNdEx)
					if err != nil {
						return err
					}
				} else if fieldNum == 2 {
					msgStartmapvalue, postmsgIndexmapvalue, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
					if err != nil {
						return err
					}
					mapvalue = &SpacePluginInstallation{}
					if err := mapvalue.UnmarshalVT(dAtA[msgStartmapvalue:postmsgIndexmapvalue]); err != nil {
						return err
					}
					iNdEx = postmsgIndexmapvalue
				} else {
					iNdEx = entryPreIndex
					iNdEx, err = protobuf_go_lite.SkipWithin(dAtA, iNdEx, postIndex)
					if err != nil {
						return err
					}
				}
			}
			m.PluginInstallations[mapkey] = mapvalue
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}

func (m *SpacePluginInstallation) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: SpacePluginInstallation: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: SpacePluginInstallation: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ManifestKeys", wireType)
			}
			var v string
			v, iNdEx, err = protobuf_go_lite.DecodeString(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.ManifestKeys = append(m.ManifestKeys, v)
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
