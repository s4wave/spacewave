...
func (this *ListVolumesResponse) EqualVT(that *ListVolumesResponse) bool {
	...
	for i := range this.Volumes {
		if equal, ok := interface{}(this.Volumes[i]).(interface{ EqualVT(*volume.VolumeInfo) bool }); ok {
			if !equal.EqualVT(that.Volumes[i]) {
				return false
			}
		} else if !proto.Equal(this.Volumes[i], that.Volumes[i]) {
			return false
		}
	}
	return string(this.unknownFields) == string(that.unknownFields)
...
func (this *ListBucketsResponse) EqualVT(that *ListBucketsResponse) bool {
	...
	for i := range this.Buckets {
		if equal, ok := interface{}(this.Buckets[i]).(interface {
			EqualVT(*volume.VolumeBucketInfo) bool
		}); ok {
			if !equal.EqualVT(that.Buckets[i]) {
				return false
			}
		} else if !proto.Equal(this.Buckets[i], that.Buckets[i]) {
			return false
		}
	}
	return string(this.unknownFields) == string(that.unknownFields)
