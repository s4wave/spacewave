...
func (m *SyncMessage) CloneVT() *SyncMessage {
	...
	r := new(SyncMessage)
	r.Transfer = protobuf_go_lite.CloneVTValue(m.Transfer)
...
func (this *SyncMessage) EqualVT(that *SyncMessage) bool {
	...
	if !protobuf_go_lite.IsEqualVT(this.Transfer, that.Transfer) {
		return false
	}
...
func (x *SyncMessage) MarshalProtoText() string {
	...
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "SyncMessage")
	if x.Transfer != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "transfer")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.Transfer)
	}
...
func (m *SyncMessage) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 1:
			...
			if err := m.Transfer.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
