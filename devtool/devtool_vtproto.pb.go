...
func (m *ProjectConfig) CloneVT() *ProjectConfig {
	...
	if rhs := m.Plugins; rhs != nil {
		tmpContainer := make(map[string]*proto.ConfigSet, len(rhs))
		for k, v := range rhs {
			if vtpb, ok := interface{}(v).(interface{ CloneVT() *proto.ConfigSet }); ok {
				tmpContainer[k] = vtpb.CloneVT()
			} else {
				tmpContainer[k] = proto1.Clone(v).(*proto.ConfigSet)
			}
		}
...
func (m *ProjectConfig) CloneGenericVT() proto1.Message {
	return m.CloneVT()
}
...
func (m *StartConfig) CloneGenericVT() proto1.Message {
	return m.CloneVT()
}
...
func (this *ProjectConfig) EqualVT(that *ProjectConfig) bool {
	...
	for i, vx := range this.Plugins {
		...
		if p, q := vx, vy; p != q {
			...
			if equal, ok := interface{}(p).(interface{ EqualVT(*proto.ConfigSet) bool }); ok {
				if !equal.EqualVT(q) {
					return false
				}
...
func (m *StartConfig) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.PluginIds) > 0 {
		for iNdEx := len(m.PluginIds) - 1; iNdEx >= 0; iNdEx-- {
			...
			i = encodeVarint(dAtA, i, uint64(len(m.PluginIds[iNdEx])))
			i--
			dAtA[i] = 0xa
		}
	}
...
func (m *ProjectConfig) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			if m.Plugins == nil {
				m.Plugins = make(map[string]*proto.ConfigSet)
			}
			var mapkey string
			var mapvalue *proto.ConfigSet
			for iNdEx < postIndex {
				entryPreIndex := iNdEx
				var wire uint64
				for shift := uint(0); ; shift += 7 {
