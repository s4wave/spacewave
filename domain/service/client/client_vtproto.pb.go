...
func (this *Config) EqualVT(that *Config) bool {
	...
	if equal, ok := interface{}(this.ClientOpts).(interface{ EqualVT(*client.Config) bool }); ok {
		if !equal.EqualVT(that.ClientOpts) {
			return false
		}
	} else if !proto.Equal(this.ClientOpts, that.ClientOpts) {
		return false
	}
...
func (m *Config) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.ClientOpts != nil {
		if vtmsg, ok := interface{}(m.ClientOpts).(interface {
			...
			encoded, err := proto.Marshal(m.ClientOpts)
			if err != nil {
				return 0, err
			}
			i -= len(encoded)
...
func (m *Config) SizeVT() (n int) {
	...
	if m.ClientOpts != nil {
		if size, ok := interface{}(m.ClientOpts).(interface {
			SizeVT() int
		}); ok {
			l = size.SizeVT()
		} else {
			l = proto.Size(m.ClientOpts)
		}
		n += 1 + l + sov(uint64(l))
	}
	l = len(m.PeerId)
...
func (m *Config) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			if unmarshal, ok := interface{}(m.ClientOpts).(interface {
				UnmarshalVT([]byte) error
			}); ok {
				if err := unmarshal.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
					return err
				...
				if err := proto.Unmarshal(dAtA[iNdEx:postIndex], m.ClientOpts); err != nil {
					return err
				}
			}
			iNdEx = postIndex
