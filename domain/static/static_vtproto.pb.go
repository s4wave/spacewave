...
func (this *Config) EqualVT(that *Config) bool {
	...
	for i := range this.Domains {
		if this.Domains[i] != that.Domains[i] {
			return false
		}
	}
	...
	for i := range this.Entities {
		if equal, ok := interface{}(this.Entities[i]).(interface{ EqualVT(*identity.Entity) bool }); ok {
			if !equal.EqualVT(that.Entities[i]) {
				return false
			}
		} else if !proto.Equal(this.Entities[i], that.Entities[i]) {
			return false
		}
	}
