...
func (m *Tx) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			if wireType != 2 {
				return errors.Errorf("proto: wrong wireType = %d for field TxSetOutputs", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
...
func (m *TxSetOutputs) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return errors.Errorf("proto: TxSetOutputs: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: TxSetOutputs: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 2:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field ClearOld", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
