...
func (m *Tx) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ClusterObjectKey) > 0 {
		i = protobuf_go_lite.EncodeString(dAtA, i, m.ClusterObjectKey)
		i--
		dAtA[i] = 0x12
	}
	if m.TxType != 0 {
...
func (m *Tx) SizeVT() (n int) {
	...
	n += protobuf_go_lite.SizeStringNonEmpty(1, m.ClusterObjectKey)
...
func (x *Tx) MarshalProtoText() string {
	...
	if x.ClusterObjectKey != "" {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "cluster_object_key")
		protobuf_go_lite.TextWriteString(&sb, x.ClusterObjectKey)
	}
