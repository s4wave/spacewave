// Code generated by protoc-gen-go-lite. DO NOT EDIT.
// protoc-gen-go-lite version: v0.18.1-0.20260826222423-298dca0e6eaf
// source: github.com/s4wave/spacewave/forge/task/tx/tx.proto

package task_tx

import (
	fmt "fmt"
	io "io"
	slices "slices"
	strconv "strconv"

	protobuf_go_lite "github.com/aperturerobotics/protobuf-go-lite"
	json "github.com/aperturerobotics/protobuf-go-lite/json"
	target "github.com/s4wave/spacewave/forge/target"
	value "github.com/s4wave/spacewave/forge/value"
)

// TxType indicates the kind of transaction.
type TxType int32

const (
	TxType_TxType_INVALID TxType = 0
	// TxType_UPDATE_INPUTS updates the ValueSet inputs and target.
	// If changed outside a live Pass, transitions to PENDING.
	// If a Pass is live, requests cancellation and stays RUNNING until drain.
	TxType_TxType_UPDATE_INPUTS TxType = 1
	// TxType_START starts the Task when every predecessor Pass is complete.
	// Transitions PENDING to RUNNING and creates the next Pass.
	TxType_TxType_START TxType = 2
	// TxType_UPDATE_WITH_PASS_STATE updates the Task from the current Pass.
	// Canceled completion transitions to PENDING; other completion to CHECKING.
	// A nonterminal Pass leaves the Task state unchanged.
	TxType_TxType_UPDATE_WITH_PASS_STATE TxType = 3
	// TxType_COMPLETE sets the result of the Task.
	// If failed, can transition from any state.
	// If success, must transition from CHECKING state.
	// If success, all Execution states must be Successful.
	TxType_TxType_COMPLETE TxType = 4
	// TxType_RETRY authorizes one successor Pass for a failed predecessor.
	// The expected pass nonce is the idempotency fence.
	TxType_TxType_RETRY TxType = 5
)

// Enum value maps for TxType.
var (
	TxType_name = map[int32]string{
		0: "TxType_INVALID",
		1: "TxType_UPDATE_INPUTS",
		2: "TxType_START",
		3: "TxType_UPDATE_WITH_PASS_STATE",
		4: "TxType_COMPLETE",
		5: "TxType_RETRY",
	}
	TxType_value = map[string]int32{
		"TxType_INVALID":                0,
		"TxType_UPDATE_INPUTS":          1,
		"TxType_START":                  2,
		"TxType_UPDATE_WITH_PASS_STATE": 3,
		"TxType_COMPLETE":               4,
		"TxType_RETRY":                  5,
	}
)

func (x TxType) Enum() *TxType {
	p := new(TxType)
	*p = x
	return p
}

func (x TxType) String() string {
	name, valid := TxType_name[int32(x)]
	if valid {
		return name
	}
	return strconv.Itoa(int(x))
}

// Tx is the on-the-wire representation of a transaction.
type Tx struct {
	unknownFields []byte
	// TxType is the kind of transaction this is.
	TxType TxType `protobuf:"varint,1,opt,name=tx_type,json=txType,proto3" json:"txType,omitempty"`
	// TaskObjectKey is the Task object ID this is associated with.
	// The Task object must already exist.
	TaskObjectKey string `protobuf:"bytes,2,opt,name=task_object_key,json=taskObjectKey,proto3" json:"taskObjectKey,omitempty"`
	// TxUpdateInputs updates the Task with the latest Target and Inputs.
	// TxType_UPDATE_INPUTS
	TxUpdateInputs *TxUpdateInputs `protobuf:"bytes,3,opt,name=tx_update_inputs,json=txUpdateInputs,proto3" json:"txUpdateInputs,omitempty"`
	// TxStart contains the start transaction tx.
	// TxType_START
	TxStart *TxStart `protobuf:"bytes,4,opt,name=tx_start,json=txStart,proto3" json:"txStart,omitempty"`
	// TxUpdatePassState contains the update pass state tx.
	// TxType_UPDATE_WITH_PASS_STATE
	TxUpdateWithPassState *TxUpdateWithPassState `protobuf:"bytes,5,opt,name=tx_update_with_pass_state,json=txUpdateWithPassState,proto3" json:"txUpdateWithPassState,omitempty"`
	// TxComplete contains the complete tx.
	// TxType_COMPLETE
	TxComplete *TxComplete `protobuf:"bytes,6,opt,name=tx_complete,json=txComplete,proto3" json:"txComplete,omitempty"`
	// TxRetry contains the explicit retry transaction.
	// TxType_RETRY
	TxRetry *TxRetry `protobuf:"bytes,7,opt,name=tx_retry,json=txRetry,proto3" json:"txRetry,omitempty"`
}

func (x *Tx) Reset() {
	*x = Tx{}
}

func (*Tx) ProtoMessage() {}

func (x *Tx) GetTxType() TxType {
	if x != nil {
		return x.TxType
	}
	return TxType_TxType_INVALID
}

func (x *Tx) GetTaskObjectKey() string {
	if x != nil {
		return x.TaskObjectKey
	}
	return ""
}

func (x *Tx) GetTxUpdateInputs() *TxUpdateInputs {
	if x != nil {
		return x.TxUpdateInputs
	}
	return nil
}

func (x *Tx) GetTxStart() *TxStart {
	if x != nil {
		return x.TxStart
	}
	return nil
}

func (x *Tx) GetTxUpdateWithPassState() *TxUpdateWithPassState {
	if x != nil {
		return x.TxUpdateWithPassState
	}
	return nil
}

func (x *Tx) GetTxComplete() *TxComplete {
	if x != nil {
		return x.TxComplete
	}
	return nil
}

func (x *Tx) GetTxRetry() *TxRetry {
	if x != nil {
		return x.TxRetry
	}
	return nil
}

// TxRetry authorizes a successor Pass after a terminal failed Pass.
// The scheduler supplies the named inputs for the successor.
type TxRetry struct {
	unknownFields []byte
	// ExpectedPassNonce identifies the failed Pass being retried.
	ExpectedPassNonce uint64 `protobuf:"varint,1,opt,name=expected_pass_nonce,json=expectedPassNonce,proto3" json:"expectedPassNonce,omitempty"`
	// NextInputs contains the named inputs for the successor Pass.
	// Outputs must be empty.
	NextInputs *target.ValueSet `protobuf:"bytes,2,opt,name=next_inputs,json=nextInputs,proto3" json:"nextInputs,omitempty"`
}

func (x *TxRetry) Reset() {
	*x = TxRetry{}
}

func (*TxRetry) ProtoMessage() {}

func (x *TxRetry) GetExpectedPassNonce() uint64 {
	if x != nil {
		return x.ExpectedPassNonce
	}
	return 0
}

func (x *TxRetry) GetNextInputs() *target.ValueSet {
	if x != nil {
		return x.NextInputs
	}
	return nil
}

// TxUpdateInputs updates the Task with the latest Target and Inputs.
// If changed outside a live Pass, transitions to PENDING.
// If a Pass is live, requests cancellation and stays RUNNING until drain.
//
// TxType: TxType_UPDATE_INPUTS
type TxUpdateInputs struct {
	unknownFields []byte
	// UpdateTarget indicates to update the TargetRef if necessary.
	UpdateTarget bool `protobuf:"varint,1,opt,name=update_target,json=updateTarget,proto3" json:"updateTarget,omitempty"`
	// ResetInputs indicates to clear the existing inputs before setting.
	ResetInputs bool `protobuf:"varint,2,opt,name=reset_inputs,json=resetInputs,proto3" json:"resetInputs,omitempty"`
	// ValueSet is the set of inputs to set for the task.
	// Outputs must be empty.
	// Any Value with Type=UNKNOWN (0) will be deleted.
	ValueSet *target.ValueSet `protobuf:"bytes,3,opt,name=value_set,json=valueSet,proto3" json:"valueSet,omitempty"`
}

func (x *TxUpdateInputs) Reset() {
	*x = TxUpdateInputs{}
}

func (*TxUpdateInputs) ProtoMessage() {}

func (x *TxUpdateInputs) GetUpdateTarget() bool {
	if x != nil {
		return x.UpdateTarget
	}
	return false
}

func (x *TxUpdateInputs) GetResetInputs() bool {
	if x != nil {
		return x.ResetInputs
	}
	return false
}

func (x *TxUpdateInputs) GetValueSet() *target.ValueSet {
	if x != nil {
		return x.ValueSet
	}
	return nil
}

// TxStart starts a Task by creating a Pass.
// Transitions PENDING to RUNNING only after every predecessor Pass is complete.
// Requests cancellation without creating a successor while a predecessor lives.
//
// TxType: TxType_START
type TxStart struct {
	unknownFields []byte
	// AssignSelf assigns the sender of the tx to the new Pass.
	AssignSelf bool `protobuf:"varint,1,opt,name=assign_self,json=assignSelf,proto3" json:"assignSelf,omitempty"`
}

func (x *TxStart) Reset() {
	*x = TxStart{}
}

func (*TxStart) ProtoMessage() {}

func (x *TxStart) GetAssignSelf() bool {
	if x != nil {
		return x.AssignSelf
	}
	return false
}

// TxUpdateWithPassState updates the Task from the current Pass.
// Canceled completion transitions to PENDING; other completion to CHECKING.
// A nonterminal Pass leaves the Task state unchanged.
// TxType: TxType_UPDATE_WITH_PASS_STATE
type TxUpdateWithPassState struct {
	unknownFields []byte
}

func (x *TxUpdateWithPassState) Reset() {
	*x = TxUpdateWithPassState{}
}

func (*TxUpdateWithPassState) ProtoMessage() {}

// TxComplete completes the execution by setting the result.
// If failed, may transition from any state.
// If success, must be in the CHECKING state.
// If success, the most recent Pass must be in COMPLETE state and not failed.
// TxType: TxType_COMPLETE
type TxComplete struct {
	unknownFields []byte
	// Result is information about the outcome of a completed pass.
	Result *value.Result `protobuf:"bytes,1,opt,name=result,proto3" json:"result,omitempty"`
	// ValueSet is the set of outputs from the task.
	// Inputs must be empty.
	// Must match the outputs calculated from the Pass and Execution objects.
	// Must be empty if the result is not success.
	ValueSet *target.ValueSet `protobuf:"bytes,2,opt,name=value_set,json=valueSet,proto3" json:"valueSet,omitempty"`
}

func (x *TxComplete) Reset() {
	*x = TxComplete{}
}

func (*TxComplete) ProtoMessage() {}

func (x *TxComplete) GetResult() *value.Result {
	if x != nil {
		return x.Result
	}
	return nil
}

func (x *TxComplete) GetValueSet() *target.ValueSet {
	if x != nil {
		return x.ValueSet
	}
	return nil
}

func (m *Tx) CloneVT() *Tx {
	if m == nil {
		return (*Tx)(nil)
	}
	r := new(Tx)
	r.TxType = m.TxType
	r.TaskObjectKey = m.TaskObjectKey
	r.TxUpdateInputs = protobuf_go_lite.CloneVTValue(m.TxUpdateInputs)
	r.TxStart = protobuf_go_lite.CloneVTValue(m.TxStart)
	r.TxUpdateWithPassState = protobuf_go_lite.CloneVTValue(m.TxUpdateWithPassState)
	r.TxComplete = protobuf_go_lite.CloneVTValue(m.TxComplete)
	r.TxRetry = protobuf_go_lite.CloneVTValue(m.TxRetry)
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *Tx) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (m *TxRetry) CloneVT() *TxRetry {
	if m == nil {
		return (*TxRetry)(nil)
	}
	r := new(TxRetry)
	r.ExpectedPassNonce = m.ExpectedPassNonce
	r.NextInputs = protobuf_go_lite.CloneVTValue(m.NextInputs)
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *TxRetry) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (m *TxUpdateInputs) CloneVT() *TxUpdateInputs {
	if m == nil {
		return (*TxUpdateInputs)(nil)
	}
	r := new(TxUpdateInputs)
	r.UpdateTarget = m.UpdateTarget
	r.ResetInputs = m.ResetInputs
	r.ValueSet = protobuf_go_lite.CloneVTValue(m.ValueSet)
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *TxUpdateInputs) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (m *TxStart) CloneVT() *TxStart {
	if m == nil {
		return (*TxStart)(nil)
	}
	r := new(TxStart)
	r.AssignSelf = m.AssignSelf
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *TxStart) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (m *TxUpdateWithPassState) CloneVT() *TxUpdateWithPassState {
	if m == nil {
		return (*TxUpdateWithPassState)(nil)
	}
	r := new(TxUpdateWithPassState)
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *TxUpdateWithPassState) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (m *TxComplete) CloneVT() *TxComplete {
	if m == nil {
		return (*TxComplete)(nil)
	}
	r := new(TxComplete)
	r.Result = protobuf_go_lite.CloneVTValue(m.Result)
	r.ValueSet = protobuf_go_lite.CloneVTValue(m.ValueSet)
	if len(m.unknownFields) > 0 {
		r.unknownFields = slices.Clone(m.unknownFields)
	}
	return r
}

func (m *TxComplete) CloneMessageVT() protobuf_go_lite.CloneMessage {
	return m.CloneVT()
}

func (this *Tx) EqualVT(that *Tx) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if this.TxType != that.TxType {
		return false
	}
	if this.TaskObjectKey != that.TaskObjectKey {
		return false
	}
	if !protobuf_go_lite.IsEqualVT(this.TxUpdateInputs, that.TxUpdateInputs) {
		return false
	}
	if !protobuf_go_lite.IsEqualVT(this.TxStart, that.TxStart) {
		return false
	}
	if !protobuf_go_lite.IsEqualVT(this.TxUpdateWithPassState, that.TxUpdateWithPassState) {
		return false
	}
	if !protobuf_go_lite.IsEqualVT(this.TxComplete, that.TxComplete) {
		return false
	}
	if !protobuf_go_lite.IsEqualVT(this.TxRetry, that.TxRetry) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *Tx) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*Tx)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

func (this *TxRetry) EqualVT(that *TxRetry) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if this.ExpectedPassNonce != that.ExpectedPassNonce {
		return false
	}
	if !protobuf_go_lite.IsEqualVT(this.NextInputs, that.NextInputs) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *TxRetry) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*TxRetry)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

func (this *TxUpdateInputs) EqualVT(that *TxUpdateInputs) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if this.UpdateTarget != that.UpdateTarget {
		return false
	}
	if this.ResetInputs != that.ResetInputs {
		return false
	}
	if !protobuf_go_lite.IsEqualVT(this.ValueSet, that.ValueSet) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *TxUpdateInputs) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*TxUpdateInputs)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

func (this *TxStart) EqualVT(that *TxStart) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if this.AssignSelf != that.AssignSelf {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *TxStart) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*TxStart)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

func (this *TxUpdateWithPassState) EqualVT(that *TxUpdateWithPassState) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *TxUpdateWithPassState) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*TxUpdateWithPassState)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

func (this *TxComplete) EqualVT(that *TxComplete) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if !protobuf_go_lite.IsEqualVT(this.Result, that.Result) {
		return false
	}
	if !protobuf_go_lite.IsEqualVT(this.ValueSet, that.ValueSet) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}

func (this *TxComplete) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*TxComplete)
	if !ok {
		return false
	}
	return this.EqualVT(that)
}

// MarshalProtoJSON marshals the TxType to JSON.
func (x TxType) MarshalProtoJSON(s *json.MarshalState) {
	s.WriteEnum(int32(x), TxType_name)
}

// MarshalText marshals the TxType to text.
func (x TxType) MarshalText() ([]byte, error) {
	return []byte(json.GetEnumString(int32(x), TxType_name)), nil
}

// MarshalJSON marshals the TxType to JSON.
func (x TxType) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the TxType from JSON.
func (x *TxType) UnmarshalProtoJSON(s *json.UnmarshalState) {
	v := s.ReadEnum(TxType_value)
	if err := s.Err(); err != nil {
		s.SetErrorf("could not read TxType enum: %v", err)
		return
	}
	*x = TxType(v)
}

// UnmarshalText unmarshals the TxType from text.
func (x *TxType) UnmarshalText(b []byte) error {
	i, err := json.ParseEnumString(string(b), TxType_value)
	if err != nil {
		return err
	}
	*x = TxType(i)
	return nil
}

// UnmarshalJSON unmarshals the TxType from JSON.
func (x *TxType) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the Tx message to JSON.
func (x *Tx) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if x.TxType != 0 || s.HasField("txType") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("txType")
		x.TxType.MarshalProtoJSON(s)
	}
	if x.TaskObjectKey != "" || s.HasField("taskObjectKey") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("taskObjectKey")
		s.WriteString(x.TaskObjectKey)
	}
	if x.TxUpdateInputs != nil || s.HasField("txUpdateInputs") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("txUpdateInputs")
		x.TxUpdateInputs.MarshalProtoJSON(s.WithField("txUpdateInputs"))
	}
	if x.TxStart != nil || s.HasField("txStart") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("txStart")
		x.TxStart.MarshalProtoJSON(s.WithField("txStart"))
	}
	if x.TxUpdateWithPassState != nil || s.HasField("txUpdateWithPassState") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("txUpdateWithPassState")
		x.TxUpdateWithPassState.MarshalProtoJSON(s.WithField("txUpdateWithPassState"))
	}
	if x.TxComplete != nil || s.HasField("txComplete") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("txComplete")
		x.TxComplete.MarshalProtoJSON(s.WithField("txComplete"))
	}
	if x.TxRetry != nil || s.HasField("txRetry") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("txRetry")
		x.TxRetry.MarshalProtoJSON(s.WithField("txRetry"))
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the Tx to JSON.
func (x *Tx) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the Tx message from JSON.
func (x *Tx) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "tx_type", "txType":
			s.AddField("tx_type")
			x.TxType.UnmarshalProtoJSON(s)
		case "task_object_key", "taskObjectKey":
			s.AddField("task_object_key")
			x.TaskObjectKey = s.ReadString()
		case "tx_update_inputs", "txUpdateInputs":
			if s.ReadNil() {
				x.TxUpdateInputs = nil
				return
			}
			x.TxUpdateInputs = &TxUpdateInputs{}
			x.TxUpdateInputs.UnmarshalProtoJSON(s.WithField("tx_update_inputs", true))
		case "tx_start", "txStart":
			if s.ReadNil() {
				x.TxStart = nil
				return
			}
			x.TxStart = &TxStart{}
			x.TxStart.UnmarshalProtoJSON(s.WithField("tx_start", true))
		case "tx_update_with_pass_state", "txUpdateWithPassState":
			if s.ReadNil() {
				x.TxUpdateWithPassState = nil
				return
			}
			x.TxUpdateWithPassState = &TxUpdateWithPassState{}
			x.TxUpdateWithPassState.UnmarshalProtoJSON(s.WithField("tx_update_with_pass_state", true))
		case "tx_complete", "txComplete":
			if s.ReadNil() {
				x.TxComplete = nil
				return
			}
			x.TxComplete = &TxComplete{}
			x.TxComplete.UnmarshalProtoJSON(s.WithField("tx_complete", true))
		case "tx_retry", "txRetry":
			if s.ReadNil() {
				x.TxRetry = nil
				return
			}
			x.TxRetry = &TxRetry{}
			x.TxRetry.UnmarshalProtoJSON(s.WithField("tx_retry", true))
		}
	})
}

// UnmarshalJSON unmarshals the Tx from JSON.
func (x *Tx) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the TxRetry message to JSON.
func (x *TxRetry) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if x.ExpectedPassNonce != 0 || s.HasField("expectedPassNonce") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("expectedPassNonce")
		s.WriteUint64(x.ExpectedPassNonce)
	}
	if x.NextInputs != nil || s.HasField("nextInputs") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("nextInputs")
		x.NextInputs.MarshalProtoJSON(s.WithField("nextInputs"))
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the TxRetry to JSON.
func (x *TxRetry) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the TxRetry message from JSON.
func (x *TxRetry) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "expected_pass_nonce", "expectedPassNonce":
			s.AddField("expected_pass_nonce")
			x.ExpectedPassNonce = s.ReadUint64()
		case "next_inputs", "nextInputs":
			if s.ReadNil() {
				x.NextInputs = nil
				return
			}
			x.NextInputs = &target.ValueSet{}
			x.NextInputs.UnmarshalProtoJSON(s.WithField("next_inputs", true))
		}
	})
}

// UnmarshalJSON unmarshals the TxRetry from JSON.
func (x *TxRetry) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the TxUpdateInputs message to JSON.
func (x *TxUpdateInputs) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if x.UpdateTarget || s.HasField("updateTarget") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("updateTarget")
		s.WriteBool(x.UpdateTarget)
	}
	if x.ResetInputs || s.HasField("resetInputs") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("resetInputs")
		s.WriteBool(x.ResetInputs)
	}
	if x.ValueSet != nil || s.HasField("valueSet") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("valueSet")
		x.ValueSet.MarshalProtoJSON(s.WithField("valueSet"))
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the TxUpdateInputs to JSON.
func (x *TxUpdateInputs) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the TxUpdateInputs message from JSON.
func (x *TxUpdateInputs) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "update_target", "updateTarget":
			s.AddField("update_target")
			x.UpdateTarget = s.ReadBool()
		case "reset_inputs", "resetInputs":
			s.AddField("reset_inputs")
			x.ResetInputs = s.ReadBool()
		case "value_set", "valueSet":
			if s.ReadNil() {
				x.ValueSet = nil
				return
			}
			x.ValueSet = &target.ValueSet{}
			x.ValueSet.UnmarshalProtoJSON(s.WithField("value_set", true))
		}
	})
}

// UnmarshalJSON unmarshals the TxUpdateInputs from JSON.
func (x *TxUpdateInputs) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the TxStart message to JSON.
func (x *TxStart) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if x.AssignSelf || s.HasField("assignSelf") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("assignSelf")
		s.WriteBool(x.AssignSelf)
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the TxStart to JSON.
func (x *TxStart) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the TxStart message from JSON.
func (x *TxStart) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "assign_self", "assignSelf":
			s.AddField("assign_self")
			x.AssignSelf = s.ReadBool()
		}
	})
}

// UnmarshalJSON unmarshals the TxStart from JSON.
func (x *TxStart) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the TxUpdateWithPassState message to JSON.
func (x *TxUpdateWithPassState) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	s.WriteObjectEnd()
}

// MarshalJSON marshals the TxUpdateWithPassState to JSON.
func (x *TxUpdateWithPassState) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the TxUpdateWithPassState message from JSON.
func (x *TxUpdateWithPassState) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		// no fields
	})
}

// UnmarshalJSON unmarshals the TxUpdateWithPassState from JSON.
func (x *TxUpdateWithPassState) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

// MarshalProtoJSON marshals the TxComplete message to JSON.
func (x *TxComplete) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if x.Result != nil || s.HasField("result") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("result")
		x.Result.MarshalProtoJSON(s.WithField("result"))
	}
	if x.ValueSet != nil || s.HasField("valueSet") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("valueSet")
		x.ValueSet.MarshalProtoJSON(s.WithField("valueSet"))
	}
	s.WriteObjectEnd()
}

// MarshalJSON marshals the TxComplete to JSON.
func (x *TxComplete) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the TxComplete message from JSON.
func (x *TxComplete) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		switch key {
		default:
			s.Skip() // ignore unknown field
		case "result":
			if s.ReadNil() {
				x.Result = nil
				return
			}
			x.Result = &value.Result{}
			x.Result.UnmarshalProtoJSON(s.WithField("result", true))
		case "value_set", "valueSet":
			if s.ReadNil() {
				x.ValueSet = nil
				return
			}
			x.ValueSet = &target.ValueSet{}
			x.ValueSet.UnmarshalProtoJSON(s.WithField("value_set", true))
		}
	})
}

// UnmarshalJSON unmarshals the TxComplete from JSON.
func (x *TxComplete) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}

func (m *Tx) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *Tx) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *Tx) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	if m.TxRetry != nil {
		size, err := m.TxRetry.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x3a
	}
	if m.TxComplete != nil {
		size, err := m.TxComplete.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x32
	}
	if m.TxUpdateWithPassState != nil {
		size, err := m.TxUpdateWithPassState.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x2a
	}
	if m.TxStart != nil {
		size, err := m.TxStart.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x22
	}
	if m.TxUpdateInputs != nil {
		size, err := m.TxUpdateInputs.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.TaskObjectKey) > 0 {
		i = protobuf_go_lite.EncodeString(dAtA, i, m.TaskObjectKey)
		i--
		dAtA[i] = 0x12
	}
	if m.TxType != 0 {
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(m.TxType))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
}

func (m *TxRetry) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *TxRetry) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *TxRetry) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	if m.NextInputs != nil {
		size, err := m.NextInputs.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	if m.ExpectedPassNonce != 0 {
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(m.ExpectedPassNonce))
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
}

func (m *TxUpdateInputs) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *TxUpdateInputs) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *TxUpdateInputs) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	if m.ValueSet != nil {
		size, err := m.ValueSet.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x1a
	}
	if m.ResetInputs {
		i = protobuf_go_lite.EncodeBool(dAtA, i, m.ResetInputs)
		i--
		dAtA[i] = 0x10
	}
	if m.UpdateTarget {
		i = protobuf_go_lite.EncodeBool(dAtA, i, m.UpdateTarget)
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
}

func (m *TxStart) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *TxStart) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *TxStart) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	if m.AssignSelf {
		i = protobuf_go_lite.EncodeBool(dAtA, i, m.AssignSelf)
		i--
		dAtA[i] = 0x8
	}
	return len(dAtA) - i, nil
}

func (m *TxUpdateWithPassState) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *TxUpdateWithPassState) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *TxUpdateWithPassState) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	return len(dAtA) - i, nil
}

func (m *TxComplete) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBufferVT(dAtA[:size])
	if err != nil {
		return nil, err
	}
	return dAtA[:n], nil
}

func (m *TxComplete) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *TxComplete) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if m.unknownFields != nil {
		i = protobuf_go_lite.EncodeRawBytes(dAtA, i, m.unknownFields)
	}
	if m.ValueSet != nil {
		size, err := m.ValueSet.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	if m.Result != nil {
		size, err := m.Result.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
		i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
}

func (m *Tx) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	n += protobuf_go_lite.SizeVarintNonZero(1, m.TxType)
	n += protobuf_go_lite.SizeStringNonEmpty(1, m.TaskObjectKey)
	if m.TxUpdateInputs != nil {
		l = m.TxUpdateInputs.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	if m.TxStart != nil {
		l = m.TxStart.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	if m.TxUpdateWithPassState != nil {
		l = m.TxUpdateWithPassState.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	if m.TxComplete != nil {
		l = m.TxComplete.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	if m.TxRetry != nil {
		l = m.TxRetry.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	n += len(m.unknownFields)
	return n
}

func (m *TxRetry) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	n += protobuf_go_lite.SizeVarintNonZero(1, m.ExpectedPassNonce)
	if m.NextInputs != nil {
		l = m.NextInputs.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	n += len(m.unknownFields)
	return n
}

func (m *TxUpdateInputs) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	n += protobuf_go_lite.SizeBoolNonZero(1, m.UpdateTarget)
	n += protobuf_go_lite.SizeBoolNonZero(1, m.ResetInputs)
	if m.ValueSet != nil {
		l = m.ValueSet.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	n += len(m.unknownFields)
	return n
}

func (m *TxStart) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	n += protobuf_go_lite.SizeBoolNonZero(1, m.AssignSelf)
	n += len(m.unknownFields)
	return n
}

func (m *TxUpdateWithPassState) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	n += len(m.unknownFields)
	return n
}

func (m *TxComplete) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	if m.Result != nil {
		l = m.Result.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	if m.ValueSet != nil {
		l = m.ValueSet.SizeVT()
		n += protobuf_go_lite.SizeMessage(1, l)
	}
	n += len(m.unknownFields)
	return n
}

func (x TxType) MarshalProtoText() string {
	return x.String()
}

func (x *Tx) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "Tx")
	if x.TxType != 0 {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "tx_type")
		protobuf_go_lite.TextWriteStringer(&sb, TxType(x.TxType))
	}
	if x.TaskObjectKey != "" {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "task_object_key")
		protobuf_go_lite.TextWriteString(&sb, x.TaskObjectKey)
	}
	if x.TxUpdateInputs != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "tx_update_inputs")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.TxUpdateInputs)
	}
	if x.TxStart != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "tx_start")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.TxStart)
	}
	if x.TxUpdateWithPassState != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "tx_update_with_pass_state")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.TxUpdateWithPassState)
	}
	if x.TxComplete != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "tx_complete")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.TxComplete)
	}
	if x.TxRetry != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "tx_retry")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.TxRetry)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *Tx) String() string {
	return x.MarshalProtoText()
}

func (x *TxRetry) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "TxRetry")
	if x.ExpectedPassNonce != 0 {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "expected_pass_nonce")
		protobuf_go_lite.TextWriteUint(&sb, x.ExpectedPassNonce)
	}
	if x.NextInputs != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "next_inputs")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.NextInputs)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *TxRetry) String() string {
	return x.MarshalProtoText()
}

func (x *TxUpdateInputs) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "TxUpdateInputs")
	if x.UpdateTarget != false {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "update_target")
		protobuf_go_lite.TextWriteBool(&sb, x.UpdateTarget)
	}
	if x.ResetInputs != false {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "reset_inputs")
		protobuf_go_lite.TextWriteBool(&sb, x.ResetInputs)
	}
	if x.ValueSet != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "value_set")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.ValueSet)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *TxUpdateInputs) String() string {
	return x.MarshalProtoText()
}

func (x *TxStart) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "TxStart")
	if x.AssignSelf != false {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "assign_self")
		protobuf_go_lite.TextWriteBool(&sb, x.AssignSelf)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *TxStart) String() string {
	return x.MarshalProtoText()
}

func (x *TxUpdateWithPassState) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	protobuf_go_lite.TextStartMessage(&sb, "TxUpdateWithPassState")
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *TxUpdateWithPassState) String() string {
	return x.MarshalProtoText()
}

func (x *TxComplete) MarshalProtoText() string {
	var sb protobuf_go_lite.TextBuilder
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "TxComplete")
	if x.Result != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "result")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.Result)
	}
	if x.ValueSet != nil {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "value_set")
		protobuf_go_lite.TextWriteTextMarshaler(&sb, x.ValueSet)
	}
	return protobuf_go_lite.TextFinishMessage(&sb)
}

func (x *TxComplete) String() string {
	return x.MarshalProtoText()
}

func (m *Tx) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: Tx: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: Tx: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field TxType", wireType)
			}
			m.TxType = 0
			var _v uint64
			_v, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
			m.TxType = TxType(_v)
			if err != nil {
				return err
			}
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TaskObjectKey", wireType)
			}
			var v string
			v, iNdEx, err = protobuf_go_lite.DecodeString(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.TaskObjectKey = v
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TxUpdateInputs", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			if m.TxUpdateInputs == nil {
				m.TxUpdateInputs = &TxUpdateInputs{}
			}
			if err := m.TxUpdateInputs.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TxStart", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			if m.TxStart == nil {
				m.TxStart = &TxStart{}
			}
			if err := m.TxStart.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TxUpdateWithPassState", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			if m.TxUpdateWithPassState == nil {
				m.TxUpdateWithPassState = &TxUpdateWithPassState{}
			}
			if err := m.TxUpdateWithPassState.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TxComplete", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			if m.TxComplete == nil {
				m.TxComplete = &TxComplete{}
			}
			if err := m.TxComplete.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 7:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field TxRetry", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			if m.TxRetry == nil {
				m.TxRetry = &TxRetry{}
			}
			if err := m.TxRetry.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}

func (m *TxRetry) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: TxRetry: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: TxRetry: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field ExpectedPassNonce", wireType)
			}
			m.ExpectedPassNonce = 0
			m.ExpectedPassNonce, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
			if err != nil {
				return err
			}
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field NextInputs", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			if m.NextInputs == nil {
				m.NextInputs = &target.ValueSet{}
			}
			if err := m.NextInputs.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}

func (m *TxUpdateInputs) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: TxUpdateInputs: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: TxUpdateInputs: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field UpdateTarget", wireType)
			}
			var v bool
			v, iNdEx, err = protobuf_go_lite.DecodeVarintBool(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.UpdateTarget = bool(v)
		case 2:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field ResetInputs", wireType)
			}
			var v bool
			v, iNdEx, err = protobuf_go_lite.DecodeVarintBool(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.ResetInputs = bool(v)
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ValueSet", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			if m.ValueSet == nil {
				m.ValueSet = &target.ValueSet{}
			}
			if err := m.ValueSet.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}

func (m *TxStart) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: TxStart: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: TxStart: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field AssignSelf", wireType)
			}
			var v bool
			v, iNdEx, err = protobuf_go_lite.DecodeVarintBool(dAtA, iNdEx)
			if err != nil {
				return err
			}
			m.AssignSelf = bool(v)
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}

func (m *TxUpdateWithPassState) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: TxUpdateWithPassState: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: TxUpdateWithPassState: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}

func (m *TxComplete) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	var err error
	for iNdEx < l {
		preIndex := iNdEx
		var wire uint64
		wire, iNdEx, err = protobuf_go_lite.DecodeVarint(dAtA, iNdEx)
		if err != nil {
			return err
		}
		fieldNum := int32(wire >> 3)
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: TxComplete: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: TxComplete: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Result", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			if m.Result == nil {
				m.Result = &value.Result{}
			}
			if err := m.Result.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ValueSet", wireType)
			}
			msgStart, postIndex, err := protobuf_go_lite.DecodeLengthDelimited(dAtA, iNdEx)
			if err != nil {
				return err
			}
			if m.ValueSet == nil {
				m.ValueSet = &target.ValueSet{}
			}
			if err := m.ValueSet.UnmarshalVT(dAtA[msgStart:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protobuf_go_lite.Skip(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if (skippy < 0) || (iNdEx+skippy) < 0 {
				return protobuf_go_lite.ErrInvalidLength
			}
			if (iNdEx + skippy) > l {
				return io.ErrUnexpectedEOF
			}
			m.unknownFields = append(m.unknownFields, dAtA[iNdEx:iNdEx+skippy]...)
			iNdEx += skippy
		}
	}

	if iNdEx > l {
		return io.ErrUnexpectedEOF
	}
	return nil
}
