...
func (m *Job) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return errors.Errorf("proto: Job: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: Job: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field JobState", wireType)
			}
			m.JobState = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
