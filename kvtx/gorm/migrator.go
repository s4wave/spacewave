...
// Migrator migrates the database.
type Migrator struct {
	migrator.Migrator
}

func (m *Migrator) RunWithoutForeignKey(fc func() error) error {
	// TODO: we do not support PRAGMA foreign_keys
	/*
		var enabled int
...
func (m Migrator) HasTable(value interface{}) bool {
	...
	m.Migrator.RunWithValue(value, func(stmt *gorm.Statement) error {
		return m.DB.Raw("SELECT count(*) FROM gorm_meta WHERE type='table' AND name=?", stmt.Table).Row().Scan(&count)
	})
	return count > 0
}
...
func (m Migrator) HasColumn(value interface{}, name string) bool {
	...
	m.Migrator.RunWithValue(value, func(stmt *gorm.Statement) error {
		...
		if name != "" {
			m.DB.Raw(
				"SELECT count(*) FROM gorm_meta WHERE type = ? AND tbl_name = ? AND (sql LIKE ? OR sql LIKE ? OR sql LIKE ?)",
	...
	return m.RunWithoutForeignKey(func() error {
		return m.RunWithValue(value, func(stmt *gorm.Statement) error {
			if field := stmt.Schema.LookUpField(name); field != nil {
				...
				m.DB.Raw("SELECT sql FROM gorm_meta WHERE type = ? AND tbl_name = ? AND name = ?", "table", stmt.Table, stmt.Table).Row().Scan(&createSQL)
...
func (m Migrator) HasIndex(value interface{}, name string) bool {
	...
	m.RunWithValue(value, func(stmt *gorm.Statement) error {
		...
		if name != "" {
			m.DB.Raw(
				"SELECT count(*) FROM gorm_meta WHERE type = ? AND tbl_name = ? AND name = ?", "index", stmt.Table, name,
			).Row().Scan(&count)
		}
		return nil
	})
...
func (m Migrator) RenameIndex(value interface{}, oldName, newName string) error {
	return m.RunWithValue(value, func(stmt *gorm.Statement) error {
		var sql string
		m.DB.Raw("SELECT sql FROM gorm_meta WHERE type = ? AND tbl_name = ? AND name = ?", "index", stmt.Table, oldName).Row().Scan(&sql)
		if sql != "" {
