// @generated by protoc-gen-es v1.9.0 with parameter "target=ts,ts_nocheck=false"
// @generated from file github.com/aperturerobotics/forge/pass/pass.proto (package forge.pass, syntax proto3)
/* eslint-disable */

import type {
  BinaryReadOptions,
  FieldList,
  JsonReadOptions,
  JsonValue,
  PartialMessage,
  PlainMessage,
} from '@bufbuild/protobuf'
import { Message, proto3, protoInt64, Timestamp } from '@bufbuild/protobuf'
import { BlockRef } from '@go/github.com/aperturerobotics/hydra/block/block_pb.js'
import { ValueSet } from '../target/target_pb.js'
import { Result } from '../value/value_pb.js'
import { State as State$1 } from '../execution/execution_pb.js'

/**
 * State contains the possible Pass states.
 *
 * @generated from enum forge.pass.State
 */
export enum State {
  /**
   * PassState_UNKNOWN is the unknown type.
   *
   * @generated from enum value: PassState_UNKNOWN = 0;
   */
  PassState_UNKNOWN = 0,

  /**
   * PassState_PENDING is the state when the pass is not yet running.
   * Transitions to RUNNING when the pass is promoted to running.
   *
   * @generated from enum value: PassState_PENDING = 1;
   */
  PassState_PENDING = 1,

  /**
   * PassState_RUNNING is the state when the executions are running.
   * ExecStates can be added, removed, updated during this state as needed.
   *
   * @generated from enum value: PassState_RUNNING = 2;
   */
  PassState_RUNNING = 2,

  /**
   * PassState_CHECKING is the state when all exec states are completed.
   * If multiple executions were scheduled, they are checked for agreement.
   * Transition to COMPLETE state on failure (disageement) or success (validation).
   *
   * @generated from enum value: PassState_CHECKING = 3;
   */
  PassState_CHECKING = 3,

  /**
   * PassState_COMPLETE is the normal terminal state of the pass.
   * This includes both success and failure termination states.
   *
   * @generated from enum value: PassState_COMPLETE = 4;
   */
  PassState_COMPLETE = 4,
}
// Retrieve enum metadata with: proto3.getEnumType(State)
proto3.util.setEnumType(State, 'forge.pass.State', [
  { no: 0, name: 'PassState_UNKNOWN' },
  { no: 1, name: 'PassState_PENDING' },
  { no: 2, name: 'PassState_RUNNING' },
  { no: 3, name: 'PassState_CHECKING' },
  { no: 4, name: 'PassState_COMPLETE' },
])

/**
 * Pass contains state for a Task pass.
 * Contains a pointer to the Target used for the pass.
 * Contains snapshots of the execution instance states.
 * Execution instances can be added / removed.
 *
 * The Pass is complete when len(exec_states) == replicas and all exec states
 * are in the completed (terminal) state, or when a fatal error occurs.
 *
 * If the Target object changes (in the world) or inputs change, a new Pass
 * should be created (these fields are immutable).
 *
 * World graph links:
 *  - parent: usually a Task which created the Pass
 *  - forge/pass-execution: all active execution instances for the pass
 * Incoming graph links:
 *  - parent: from the Execution for the pass.
 *
 * @generated from message forge.pass.Pass
 */
export class Pass extends Message<Pass> {
  /**
   * PassState is the current state of the pass.
   *
   * @generated from field: forge.pass.State pass_state = 1;
   */
  passState = State.PassState_UNKNOWN

  /**
   * PeerId is the Pass controller peer ID.
   * Usually the peer ID of the Cluster controller managing this Pass.
   * Can be empty.
   *
   * @generated from field: string peer_id = 2;
   */
  peerId = ''

  /**
   * TargetRef is the block reference to the Target for the pass.
   *
   * @generated from field: block.BlockRef target_ref = 3;
   */
  targetRef?: BlockRef

  /**
   * ValueSet is the set of inputs and outputs used in the pass.
   * The input set is resolved before creating the Pass object.
   * The inputs are copied to the Pass objects.
   * The output set is updated when transitioning from CHECKING -> COMPLETE.
   *
   * @generated from field: forge.target.ValueSet value_set = 4;
   */
  valueSet?: ValueSet

  /**
   * Result is information about the outcome of a completed Pass.
   *
   * @generated from field: forge.value.Result result = 5;
   */
  result?: Result

  /**
   * Replicas is the configured number of executions for the Pass.
   *
   * len(replicas) must match len(exec_states) to complete
   *
   * @generated from field: uint32 replicas = 6;
   */
  replicas = 0

  /**
   * PassNonce is the nonce of the pass set by the pass creator.
   *
   * @generated from field: uint64 pass_nonce = 7;
   */
  passNonce = protoInt64.zero

  /**
   * ExecStates contains the most recent snapshot of the execution states.
   * Updated when:
   *  - PENDING to RUNNING: contains initial execution states (PENDING)
   *  - RUNNING: can add and remove execution states as needed.
   *  - RUNNING to CHECKING or COMPLETE: contains final states (COMPLETE)
   *  - Any to PENDING: cleared (set to len=0).
   *
   * @generated from field: repeated forge.pass.ExecState exec_states = 8;
   */
  execStates: ExecState[] = []

  /**
   * Timestamp is the time the Pass was created.
   * Used as a reference timestamp to make all ops deterministic.
   * For example: all unixfs timestamps will be set to this value.
   * Must be set.
   *
   * @generated from field: google.protobuf.Timestamp timestamp = 9;
   */
  timestamp?: Timestamp

  constructor(data?: PartialMessage<Pass>) {
    super()
    proto3.util.initPartial(data, this)
  }

  static readonly runtime: typeof proto3 = proto3
  static readonly typeName = 'forge.pass.Pass'
  static readonly fields: FieldList = proto3.util.newFieldList(() => [
    { no: 1, name: 'pass_state', kind: 'enum', T: proto3.getEnumType(State) },
    { no: 2, name: 'peer_id', kind: 'scalar', T: 9 /* ScalarType.STRING */ },
    { no: 3, name: 'target_ref', kind: 'message', T: BlockRef },
    { no: 4, name: 'value_set', kind: 'message', T: ValueSet },
    { no: 5, name: 'result', kind: 'message', T: Result },
    { no: 6, name: 'replicas', kind: 'scalar', T: 13 /* ScalarType.UINT32 */ },
    { no: 7, name: 'pass_nonce', kind: 'scalar', T: 4 /* ScalarType.UINT64 */ },
    {
      no: 8,
      name: 'exec_states',
      kind: 'message',
      T: ExecState,
      repeated: true,
    },
    { no: 9, name: 'timestamp', kind: 'message', T: Timestamp },
  ])

  static fromBinary(
    bytes: Uint8Array,
    options?: Partial<BinaryReadOptions>,
  ): Pass {
    return new Pass().fromBinary(bytes, options)
  }

  static fromJson(
    jsonValue: JsonValue,
    options?: Partial<JsonReadOptions>,
  ): Pass {
    return new Pass().fromJson(jsonValue, options)
  }

  static fromJsonString(
    jsonString: string,
    options?: Partial<JsonReadOptions>,
  ): Pass {
    return new Pass().fromJsonString(jsonString, options)
  }

  static equals(
    a: Pass | PlainMessage<Pass> | undefined,
    b: Pass | PlainMessage<Pass> | undefined,
  ): boolean {
    return proto3.util.equals(Pass, a, b)
  }
}

/**
 * ExecState contains the previous snapshot of an execution state.
 *
 * @generated from message forge.pass.ExecState
 */
export class ExecState extends Message<ExecState> {
  /**
   * ObjectKey is the object key of the execution instance.
   * Must exist before adding to Pass state.
   * Graph quad also exists: <pass> <pass/execution> <object_key>
   *
   * @generated from field: string object_key = 1;
   */
  objectKey = ''

  /**
   * ExecutionState is the current state of the execution.
   *
   * @generated from field: forge.execution.State execution_state = 2;
   */
  executionState = State$1.ExecutionState_UNKNOWN

  /**
   * PeerId is the identifier of the peer assigned to the execution.
   * Can be empty.
   *
   * @generated from field: string peer_id = 3;
   */
  peerId = ''

  /**
   * Timestamp is the time the parent object (usually Pass) was created.
   *
   * @generated from field: google.protobuf.Timestamp timestamp = 4;
   */
  timestamp?: Timestamp

  /**
   * ValueSet is the set of inputs and outputs used in the execution.
   * Outputs are updated when the execution reaches COMPLETE state.
   *
   * @generated from field: forge.target.ValueSet value_set = 5;
   */
  valueSet?: ValueSet

  /**
   * Result is information about the outcome of the execution.
   *
   * @generated from field: forge.value.Result result = 6;
   */
  result?: Result

  constructor(data?: PartialMessage<ExecState>) {
    super()
    proto3.util.initPartial(data, this)
  }

  static readonly runtime: typeof proto3 = proto3
  static readonly typeName = 'forge.pass.ExecState'
  static readonly fields: FieldList = proto3.util.newFieldList(() => [
    { no: 1, name: 'object_key', kind: 'scalar', T: 9 /* ScalarType.STRING */ },
    {
      no: 2,
      name: 'execution_state',
      kind: 'enum',
      T: proto3.getEnumType(State$1),
    },
    { no: 3, name: 'peer_id', kind: 'scalar', T: 9 /* ScalarType.STRING */ },
    { no: 4, name: 'timestamp', kind: 'message', T: Timestamp },
    { no: 5, name: 'value_set', kind: 'message', T: ValueSet },
    { no: 6, name: 'result', kind: 'message', T: Result },
  ])

  static fromBinary(
    bytes: Uint8Array,
    options?: Partial<BinaryReadOptions>,
  ): ExecState {
    return new ExecState().fromBinary(bytes, options)
  }

  static fromJson(
    jsonValue: JsonValue,
    options?: Partial<JsonReadOptions>,
  ): ExecState {
    return new ExecState().fromJson(jsonValue, options)
  }

  static fromJsonString(
    jsonString: string,
    options?: Partial<JsonReadOptions>,
  ): ExecState {
    return new ExecState().fromJsonString(jsonString, options)
  }

  static equals(
    a: ExecState | PlainMessage<ExecState> | undefined,
    b: ExecState | PlainMessage<ExecState> | undefined,
  ): boolean {
    return proto3.util.equals(ExecState, a, b)
  }
}
