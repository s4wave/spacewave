...
type SRPCDeviceResourceServiceClient interface {
	...
	AccessCapability(ctx context.Context, in *AccessCapabilityRequest) (*AccessCapabilityResponse, error)
}
...
func (c *srpcDeviceResourceServiceClient) AccessCapability(ctx context.Context, in *AccessCapabilityRequest) (*AccessCapabilityResponse, error) {
	out := new(AccessCapabilityResponse)
	err := c.cc.ExecCall(ctx, c.serviceID, "AccessCapability", in, out)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type SRPCDeviceResourceServiceServer interface {
	...
	// AccessCapability resolves an available plugin-published capability by ID
	// and returns a retained child resource serving its protocol.
	AccessCapability(context.Context, *AccessCapabilityRequest) (*AccessCapabilityResponse, error)
}
...
func (d *SRPCDeviceResourceServiceHandler) InvokeMethod(
	...
	case "AccessCapability":
		return true, d.InvokeMethod_AccessCapability(d.impl, strm)
	default:
		return false, nil
	}
}
...
func (SRPCDeviceResourceServiceHandler) InvokeMethod_AccessCapability(impl SRPCDeviceResourceServiceServer, strm srpc.Stream) error {
	req := new(AccessCapabilityRequest)
	if err := strm.MsgRecv(req); err != nil {
		return err
	}
	out, err := impl.AccessCapability(strm.Context(), req)
	if err != nil {
		return err
	}
	return strm.MsgSend(out)
...
type SRPCDeviceResourceService_AccessCapabilityStream interface {
	srpc.Stream
}

type srpcDeviceResourceService_AccessCapabilityStream struct {
	srpc.Stream
}
