...
func (this *SqlTableViewSetRootOp) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*SqlTableViewSetRootOp)
	if !ok {
		return false
	}
...
func (x *SqlTableViewSetRootOp) MarshalProtoText() string {
	...
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "SqlTableViewSetRootOp")
	if x.ObjectKey != "" {
...
func (m *SqlTableViewSetRootOp) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return fmt.Errorf("proto: SqlTableViewSetRootOp: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: SqlTableViewSetRootOp: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
