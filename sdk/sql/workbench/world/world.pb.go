...
func (this *SqlWorkbenchSetRootOp) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*SqlWorkbenchSetRootOp)
	if !ok {
		return false
	}
...
func (x *SqlWorkbenchSetRootOp) MarshalProtoText() string {
	...
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "SqlWorkbenchSetRootOp")
	if x.ObjectKey != "" {
...
func (m *SqlWorkbenchSetRootOp) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return fmt.Errorf("proto: SqlWorkbenchSetRootOp: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: SqlWorkbenchSetRootOp: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
