...
func TestConnectTerminalChallengesAndRemembersAcceptedUnknownSshHostKey(t *testing.T) {
	...
	if challenge.GetSshTrustHost() != host {
		t.Fatalf("challenge host = %q, want %q", challenge.GetSshTrustHost(), host)
	}
	if challenge.GetSshTrustAlgorithm() == "" {
		t.Fatal("challenge missing algorithm")
	}
	if challenge.GetSshTrustSha256Fingerprint() == "" {
		t.Fatal("challenge missing SHA256 fingerprint")
	}
	if challenge.GetSshTrustPublicKey() == "" {
		t.Fatal("challenge missing public key")
	}
	if !terminalFramesContainOutput(frames, "ssh ready\n") {
		t.Fatalf("sent frames missing SSH output: %#v", frames)
	}
...
func TestConnectTerminalRejectsUnknownSshHostKeyWithoutRemembering(t *testing.T) {
	...
	if got := len(sshHost.GetHostKeyPins()); got != 0 {
		t.Fatalf("remembered host key pins = %d, want 0", got)
	}
}
	...
	if s.hostKeyTrustResponse != nil &&
		...
		go func() {
			s.recv <- &s4wave_terminal.TerminalFrame{
				Kind:             s4wave_terminal.TerminalFrameKind_TERMINAL_FRAME_KIND_SSH_HOST_KEY_TRUST_RESPONSE,
...
func terminalFramesContainKind(frames []*s4wave_terminal.TerminalFrame, kind s4wave_terminal.TerminalFrameKind) bool {
	return terminalFrameOfKind(frames, kind) != nil
}

func terminalFrameOfKind(frames []*s4wave_terminal.TerminalFrame, kind s4wave_terminal.TerminalFrameKind) *s4wave_terminal.TerminalFrame {
