...
func (m *V86WizardConfig) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.CdnSourceObjectKey) > 0 {
		i = protobuf_go_lite.EncodeString(dAtA, i, m.CdnSourceObjectKey)
		i--
		dAtA[i] = 0x3a
	}
	if m.Source != 0 {
...
func (m *V86WizardConfig) SizeVT() (n int) {
	...
	n += protobuf_go_lite.SizeStringNonEmpty(1, m.CdnSourceObjectKey)
...
func (x *V86WizardConfig) MarshalProtoText() string {
	...
	initialLen := protobuf_go_lite.TextStartMessage(&sb, "V86WizardConfig")
	if x.Name != "" {
	...
	if x.Source != 0 {
		...
		protobuf_go_lite.TextWriteStringer(&sb, V86WizardConfig_Source(x.Source))
	}
	if x.CdnSourceObjectKey != "" {
		protobuf_go_lite.TextWriteFieldPrefix(&sb, initialLen, "cdn_source_object_key")
		protobuf_go_lite.TextWriteString(&sb, x.CdnSourceObjectKey)
	}
	if x.CdnId != "" {
