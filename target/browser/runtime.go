...
func NewRuntime(ctx context.Context, le *logrus.Entry, b bus.Bus, handler web_runtime.WebRuntimeHandler, runtimeID string, messagePort js.Value) (r *Runtime, rerr error) {
	defer func() {
		if err := recover(); err != nil {
			rerr = err.(error)
		}
	}()
	// wrap the message port into a ipc
	ch, err := ipc_message_port.NewMessagePort(ctx, messagePort)
	if err != nil {
		return nil, err
	}
	return web_runtime.NewRemote(le, b, handler, runtimeID, ch)
}
