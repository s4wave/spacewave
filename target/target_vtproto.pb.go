...
func (m *InputWorld) CloneVT() *InputWorld {
	...
	r := &InputWorld{
		EngineId:        m.EngineId,
		LookupImmediate: m.LookupImmediate,
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (m *Output) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ExecOutput) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.ExecOutput)))
		i--
		dAtA[i] = 0x1a
	}
	if m.OutputType != 0 {
		i = encodeVarint(dAtA, i, uint64(m.OutputType))
		i--
		dAtA[i] = 0x10
	}
	if len(m.Name) > 0 {
