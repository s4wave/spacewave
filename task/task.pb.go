...
func (m *Task) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return errors.Errorf("proto: Task: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: Task: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field TaskState", wireType)
			}
			m.TaskState = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.TaskState |= State(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
