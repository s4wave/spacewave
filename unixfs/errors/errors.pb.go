...
func (m *UnixFSError) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return errors.Errorf("proto: UnixFSError: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: UnixFSError: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field ErrorType", wireType)
			}
			m.ErrorType = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
		...
		case 2:
			if wireType != 2 {
				return errors.Errorf("proto: wrong wireType = %d for field ErrorBody", wireType)
			}
