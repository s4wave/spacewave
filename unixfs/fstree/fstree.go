...
func newTxFSTree(node *Node, bcs *block.Cursor) *FSTree {
	// btx = nil
	return &FSTree{
		node: node,
		bcs:  bcs,
...
// GetNode returns the node object at f.
func (f *FSTree) GetNode() *Node {
	return f.node
}
...
func (f *FSTree) Mknod(
	...
	return newTxFSTree(dnode, dcs), nil
}
...
func (f *FSTree) Mkdir(dirs ...string) (map[string]*FSTree, error) {
	...
	for i := 0; i < len(dirs); i++ {
		...
		outputCursors[dirs[i]] = newTxFSTree(dnode, dcs)
	}
	dslice.SortDirents()
...
func (f *FSTree) Remove(
	...
	if f.GetNode().GetNodeType() != NodeType_NodeType_DIRECTORY {
