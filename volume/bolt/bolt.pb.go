...
func (x *Config) MarshalProtoText() string {
	...
	if x.FreelistSync {
		sb.WriteString(" freelist_sync: ")
		sb.WriteString(strconv.FormatBool(x.FreelistSync))
	}
	if x.NoWriteKey {
...
func (m *Config) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 8:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field Sync", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
		...
		case 9:
			if wireType != 0 {
				return errors.Errorf("proto: wrong wireType = %d for field FreelistSync", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
