...
// https://github.com/esm-dev/esm.sh/blob/3324cc/packages/esm-node-services/cjs-lexer.js
import fs from 'fs'
...
const resolve = promisify(
  enhancedResolve.create({
    ...
    extensions: ['.cjs', '.js', '.json', '.node', '.es'],
  }),
)
...
function verifyExports(names) {
  ...
  const exports = Array.from(
    new Set(
      names.filter(
        (name) => identRegexp.test(name) && !reservedWords.has(name),
      ),
    ),
  )
  return {
...
const parseCjsExports = async (input) => {
  const { buildDir, importPath, nodeEnv = 'production', requireMode } = input
  const entry =
    importPath.startsWith('/') && /\.(js|cjs|mjs)$/.test(importPath)
      ? importPath
      : await resolve(buildDir, importPath)
  const exports = []
...
async function getExports(modulePath) {
  return await parseCjsExports({ buildDir, importPath: modulePath })
}
...
// Expect the module path as a command-line argument.
// cat determine-cjs-exports.mjs | node --input-type=module - 'react'
const modulePath = process.argv[2] || '.'
try {
  console.log(JSON.stringify(await getExports(modulePath)))
} catch (e) {
  let errStr = `${e}`.trim()
  errStr = trimPrefix(errStr, 'Error: ')
  if (errStr.startsWith("Can't")) {
    errStr = 'cannot' + trimPrefix(errStr, "Can't")
  }
  console.log(JSON.stringify({ error: errStr, stack: e.stack }))
}
