...
func (m *GetInfoResponse) CloneVT() *GetInfoResponse {
	...
	if rhs := m.Info; rhs != nil {
		if vtpb, ok := interface{}(rhs).(interface{ CloneVT() *pkg.WebPkgInfo }); ok {
			r.Info = vtpb.CloneVT()
		} else {
			r.Info = proto.Clone(rhs).(*pkg.WebPkgInfo)
		}
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
...
func (this *GetInfoResponse) EqualVT(that *GetInfoResponse) bool {
	...
	if equal, ok := interface{}(this.Info).(interface{ EqualVT(*pkg.WebPkgInfo) bool }); ok {
		if !equal.EqualVT(that.Info) {
			return false
		}
	} else if !proto.Equal(this.Info, that.Info) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
