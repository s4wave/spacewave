...
func (r *Remote) GetWebViews(ctx context.Context) ([]WebView, error) {
	var out []WebView
	err := r.waitState(ctx, func(s *rState) error {
		out = make([]WebView, len(s.webViews))
...
func (r *Remote) CreateWebView(ctx context.Context) (WebView, error) {
	var out WebView
	err := r.waitState(ctx, func(s *rState) error {
		var err error
