...
export const WebWorkerWasmPluginInit = {
  encode(
    message: WebWorkerWasmPluginInit,
    ...
    Object.entries(message.env).forEach(([key, value]) => {
      WebWorkerWasmPluginInit_EnvEntry.encode(
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 3:
          ...
          const entry3 = WebWorkerWasmPluginInit_EnvEntry.decode(
            reader,
            reader.uint32(),
          )
  ...
  fromJSON(object: any): WebWorkerWasmPluginInit {
    return {
      entrypoint: isSet(object.entrypoint)
        ...
        : '',
      argv: globalThis.Array.isArray(object?.argv)
        ? object.argv.map((e: any) => globalThis.String(e))
        : [],
      env: isObject(object.env)
        ? Object.entries(object.env).reduce<{ [key: string]: string }>(
            (acc, [key, value]) => {
  ...
  create<I extends Exact<DeepPartial<WebWorkerWasmPluginInit>, I>>(
    base?: I,
  ): WebWorkerWasmPluginInit {
    return WebWorkerWasmPluginInit.fromPartial(base ?? ({} as any))
  },
  fromPartial<I extends Exact<DeepPartial<WebWorkerWasmPluginInit>, I>>(
    object: I,
  ): WebWorkerWasmPluginInit {
    const message = createBaseWebWorkerWasmPluginInit()
    message.entrypoint = object.entrypoint ?? ''
    message.argv = object.argv?.map((e) => e) || []
    message.env = Object.entries(object.env ?? {}).reduce<{
      [key: string]: string
    }>((acc, [key, value]) => {
      if (value !== undefined) {
...
export const WebWorkerWasmPluginInit_EnvEntry = {
  encode(
    message: WebWorkerWasmPluginInit_EnvEntry,
    writer: _m0.Writer = _m0.Writer.create(),
  ): _m0.Writer {
    if (message.key !== '') {
  ...
  async *encodeTransform(
    source:
      | AsyncIterable<
          WebWorkerWasmPluginInit_EnvEntry | WebWorkerWasmPluginInit_EnvEntry[]
        >
      | Iterable<
          WebWorkerWasmPluginInit_EnvEntry | WebWorkerWasmPluginInit_EnvEntry[]
        >,
  ...
  fromPartial<
    I extends Exact<DeepPartial<WebWorkerWasmPluginInit_EnvEntry>, I>,
  >(object: I): WebWorkerWasmPluginInit_EnvEntry {
    const message = createBaseWebWorkerWasmPluginInit_EnvEntry()
