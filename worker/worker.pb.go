...
func (m *Worker) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return errors.Errorf("proto: Worker: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: Worker: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
...
func (m *WorkerCreateOp) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return errors.Errorf("proto: WorkerCreateOp: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: WorkerCreateOp: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 3:
			if wireType != 2 {
				return errors.Errorf("proto: wrong wireType = %d for field Keypairs", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
