...
func (this *WorkerCreateOp) EqualVT(that *WorkerCreateOp) bool {
	...
	for i := range this.Keypairs {
		if equal, ok := interface{}(this.Keypairs[i]).(interface{ EqualVT(*identity.Keypair) bool }); ok {
			if !equal.EqualVT(that.Keypairs[i]) {
				return false
			}
		} else if !proto.Equal(this.Keypairs[i], that.Keypairs[i]) {
			return false
		}
	}
	return string(this.unknownFields) == string(that.unknownFields)
