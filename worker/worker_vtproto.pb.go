...
func (m *Worker) CloneGenericVT() proto.Message {
	return m.CloneVT()
}
...
func (m *WorkerCreateOp) CloneGenericVT() proto.Message {
	return m.CloneVT()
}
...
func (this *WorkerCreateOp) EqualVT(that *WorkerCreateOp) bool {
	...
	for i, vx := range this.Keypairs {
		vy := that.Keypairs[i]
		if equal, ok := interface{}(vx).(interface{ EqualVT(*identity.Keypair) bool }); ok {
			if !equal.EqualVT(vy) {
				return false
			}
